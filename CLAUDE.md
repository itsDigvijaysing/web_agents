...
We use a thin wrapper around CDP called cdp-use: https://github.com/web-agent/cdp-use. cdp-use only provides shallow typed interfaces for the websocket calls, all CDP client and session management + other CDP helpers still live in web_agent/browser/session.py.

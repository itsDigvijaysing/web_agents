...
load_dotenv()
from web_agent import Agent, AgentHistoryList, BrowserProfile, BrowserSession, Chatwebagent
...
async def run_single_task(task_file):
	...
	try:
		...
		for logger_name in ['web_agent', 'telemetry', 'message_manager']:
		...
		if not api_key:
			print('[SKIP] web_agent_API_KEY is not set - skipping task evaluation', file=sys.stderr)
			return {
		...
		agent_llm = Chatwebagent(api_key=api_key)

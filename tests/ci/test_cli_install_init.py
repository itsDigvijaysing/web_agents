"""
Tests for web-agent CLI install and init commands.
...
def test_install_command_help():
	...
	result = subprocess.run(
		[sys.executable, '-m', 'web_agent.skill_cli.main', '--help'],
		capture_output=True,
		text=True,
	)
...
def test_init_command_help():
	...
	assert 'Generate web-agent template file' in result.stdout


def test_init_subcommand_help():
	"""Test that the init subcommand has its own help."""
	result = subprocess.run(
		[sys.executable, '-m', 'web_agent.skill_cli.main', 'init', '--help'],
		capture_output=True,
		text=True,
	)
	assert result.returncode == 0
...
def test_init_list_templates():
	...
	result = subprocess.run(
		[sys.executable, '-m', 'web_agent.skill_cli.main', 'init', '--list'],
		capture_output=True,
		text=True,
	)
	assert result.returncode == 0

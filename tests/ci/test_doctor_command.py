...
def test_check_package_installed():
	"""Test _check_package returns ok when web-agent is installed."""
	# web-agent is always installed in the test environment
	result = doctor._check_package()
	assert result['status'] == 'ok'
	assert 'web-agent' in result['message']
...
def test_check_api_key_with_env_var(monkeypatch):
	...
	monkeypatch.setenv('web_agent_API_KEY', 'test_key_12345')
...
def test_check_api_key_missing(monkeypatch):
	...
	with tempfile.TemporaryDirectory() as tmpdir:
		...
		from web_agent.skill_cli import api_key

...
This tests the fix for: When using web-agent with remote browser services that require
...
async def test_cdp_client_headers_passed_on_connect():
	...
	with patch('web_agent.browser.session.CDPClient') as mock_cdp_client_class:
		...
		# Mock SessionManager (imported inside connect() from web_agent.browser.session_manager)
		with patch('web_agent.browser.session_manager.SessionManager') as mock_session_manager_class:
...
async def test_headers_used_for_json_version_endpoint():
	...
	with patch('web_agent.browser.session.httpx.AsyncClient') as mock_client_class:
		...
		with patch('web_agent.browser.session.CDPClient') as mock_cdp_client_class:
			...
			with patch('web_agent.browser.session_manager.SessionManager') as mock_sm_class:
				mock_sm = MagicMock()

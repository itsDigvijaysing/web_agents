...
from web_agent.browser.profile import (
	webagent_DEFAULT_CHANNEL,
	BrowserChannel,
	BrowserProfile,
)
	...
	async def test_user_data_dir_not_allowed_to_corrupt_default_profile(self):
		...
		session = BrowserSession(
			browser_profile=BrowserProfile(
				headless=True,
				user_data_dir=CONFIG.web_agent_DEFAULT_USER_DATA_DIR,
				channel=webagent_DEFAULT_CHANNEL,  # chromium
				keep_alive=False,
			),
		)
		...
		try:
			...
			assert session.browser_profile.user_data_dir == CONFIG.web_agent_DEFAULT_USER_DATA_DIR
		finally:
			await session.kill()
		...
		assert profile2.user_data_dir != CONFIG.web_agent_DEFAULT_USER_DATA_DIR
		assert 'web-agent-user-data-dir-' in str(profile2.user_data_dir)
		...
		assert profile3.user_data_dir != CONFIG.web_agent_DEFAULT_USER_DATA_DIR
		assert profile3.user_data_dir == CONFIG.web_agent_DEFAULT_USER_DATA_DIR.parent / 'default-msedge'
		assert 'web-agent-user-data-dir-' not in str(profile3.user_data_dir)
...
class TestBrowserSessionReusePatterns:
	...
	async def test_sequential_agents_same_profile_same_browser(self, mock_llm):
		...
		from web_agent import Agent, BrowserSession
...
class TestBrowserSessionEventSystem:
	...
	async def test_direct_event_dispatching(self, browser_session):
		...
		from web_agent.browser.events import BrowserConnectedEvent, BrowserStartEvent
	...
	async def test_event_system_error_handling(self, browser_session):
		...
		from web_agent.browser.events import BrowserStartEvent
	...
	async def test_concurrent_event_dispatching(self, browser_session: BrowserSession):
		...
		from web_agent.browser.events import ScreenshotEvent
	...
	# 	from web_agent import BrowserSession
	...
	# 			BrowserSession(
	# 				browser_profile=BrowserProfile(
	# 					user_data_dir=Path(tempfile.mkdtemp(prefix=f'webagent-tmp-{i}')),
	# 					headless=True,
	# 					keep_alive=True,
	# 				),
	# 			)

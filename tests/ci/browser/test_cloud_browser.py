...
from web_agent.browser.cloud.cloud import (
	CloudBrowserAuthError,
	CloudBrowserClient,
	CloudBrowserError,
)
from web_agent.browser.cloud.views import CreateBrowserRequest
...
def temp_config_dir(monkeypatch):
	...
	with tempfile.TemporaryDirectory() as tmpdir:
		temp_dir = Path(tmpdir) / '.config' / 'webagent'
		temp_dir.mkdir(parents=True, exist_ok=True)

		# Use monkeypatch to set the environment variable
		monkeypatch.setenv('web_agent_CONFIG_DIR', str(temp_dir))
...
class TestCloudBrowserClient:
	...
	async def test_create_browser_success(self, mock_auth_config, monkeypatch):
		...
		monkeypatch.delenv('web_agent_API_KEY', raising=False)

		# Mock response data matching the API
		mock_response_data = {
			'id': 'test-browser-id',
			'status': 'active',
			'liveUrl': 'https://live.web-agent.com?wss=test',
		...
		with patch('httpx.AsyncClient') as mock_client_class:
			...
			assert 'X-web-agent-API-Key' in call_args.kwargs['headers']
			assert call_args.kwargs['headers']['X-web-agent-API-Key'] == 'test-token'
	...
	async def test_create_browser_auth_error(self, temp_config_dir, monkeypatch):
		...
		assert 'web_agent_API_KEY environment variable' in str(exc_info.value)
	...
	async def test_create_browser_with_env_var(self, temp_config_dir, monkeypatch):
		"""Test cloud browser creation using web_agent_API_KEY environment variable."""

		# Set environment variable
		monkeypatch.setenv('web_agent_API_KEY', 'env-test-token')
		...
		with patch('httpx.AsyncClient') as mock_client_class:
			...
			call_args = mock_client.post.call_args
			assert 'X-web-agent-API-Key' in call_args.kwargs['headers']
			assert call_args.kwargs['headers']['X-web-agent-API-Key'] == 'env-test-token'

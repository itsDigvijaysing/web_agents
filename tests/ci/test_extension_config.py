...
class TestDisableExtensionsEnvVar:
	"""Test web_agent_DISABLE_EXTENSIONS environment variable."""

	def test_default_value_is_true(self):
		"""Without env var set, enable_default_extensions should default to True."""
		# Clear the env var if it exists
		original = os.environ.pop('web_agent_DISABLE_EXTENSIONS', None)
		try:
			# Import fresh to get the default
			from web_agent.browser.profile import _get_enable_default_extensions_default

			assert _get_enable_default_extensions_default() is True
		finally:
			if original is not None:
				os.environ['web_agent_DISABLE_EXTENSIONS'] = original
	...
	def test_env_var_values(self, env_value: str, expected_enabled: bool):
		...
		original = os.environ.get('web_agent_DISABLE_EXTENSIONS')
		try:
			os.environ['web_agent_DISABLE_EXTENSIONS'] = env_value
			from web_agent.browser.profile import _get_enable_default_extensions_default
			...
			if original is not None:
				...
				os.environ.pop('web_agent_DISABLE_EXTENSIONS', None)
	...
	def test_browser_profile_uses_env_var(self):
		...
		try:
			...
			os.environ['web_agent_DISABLE_EXTENSIONS'] = 'true'

			from web_agent.browser.profile import BrowserProfile

			profile = BrowserProfile(headless=True)
			assert profile.enable_default_extensions is False, (
				'BrowserProfile should disable extensions when web_agent_DISABLE_EXTENSIONS=true'
			)

			# Test with env var set to false (enable extensions)
			os.environ['web_agent_DISABLE_EXTENSIONS'] = 'false'
			profile2 = BrowserProfile(headless=True)
			assert profile2.enable_default_extensions is True, (
				'BrowserProfile should enable extensions when web_agent_DISABLE_EXTENSIONS=false'
			)
	...
	def test_browser_session_uses_env_var(self):
		...
		original = os.environ.get('web_agent_DISABLE_EXTENSIONS')
		try:
			os.environ['web_agent_DISABLE_EXTENSIONS'] = '1'

			from web_agent.browser import BrowserSession

			session = BrowserSession(headless=True)
			assert session.browser_profile.enable_default_extensions is False, (
				'BrowserSession should disable extensions when web_agent_DISABLE_EXTENSIONS=1'
			)

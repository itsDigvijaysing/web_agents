...
class TestChatwebagentRetries:
	"""Test retry logic for Chatwebagent."""

	@pytest.fixture
	def mock_env(self, monkeypatch):
		"""Set up environment for Chatwebagent."""
		monkeypatch.setenv('web_agent_API_KEY', 'test-api-key')
	...
	async def test_retries_on_503_with_exponential_backoff(self, mock_env):
		...
		with patch('httpx.AsyncClient') as mock_client_class:
			...
			client = Chatwebagent(retry_base_delay=0.1, retry_max_delay=1.0)
	...
	async def test_no_retry_on_401(self, mock_env):
		...
		with patch('httpx.AsyncClient') as mock_client_class:
			...
			client = Chatwebagent(retry_base_delay=0.01)
	...
	async def test_max_retries_exhausted(self, mock_env):
		...
		with patch('httpx.AsyncClient') as mock_client_class:
			...
			client = Chatwebagent(max_retries=3, retry_base_delay=0.01)
...
class TestChatGoogleRetries:
	...
	async def test_retries_on_503_with_exponential_backoff(self, mock_env):
		...
		with patch('web_agent.llm.google.chat.genai') as mock_genai:

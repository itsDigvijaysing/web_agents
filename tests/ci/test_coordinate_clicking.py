...
This feature allows certain models (Claude Sonnet 4, Claude Opus 4, Gemini 3 Pro, web-agent/* models)
...
from web_agent.tools.views import ClickElementAction, ClickElementActionIndexOnly
...
class TestCoordinateClickingModelDetection:
	...
	@pytest.mark.parametrize(
		...
		[
			# Models that SHOULD have coordinate clicking (claude-sonnet-4*, claude-opus-4*, gemini-3-pro*, web-agent/*)
	...
	def test_model_detection_patterns(self, model_name: str, expected_coords: bool):
		...
		supports_coords = any(
			pattern in model_lower for pattern in ['claude-sonnet-4', 'claude-opus-4', 'gemini-3-pro', 'web-agent/']
		)

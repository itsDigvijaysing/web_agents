...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	"""Setup logging configuration for web-agent.

	Args:
		stream: Output stream for logs (default: sys.stdout). Can be sys.stderr for MCP mode.
		log_level: Override log level (default: uses CONFIG.web_agent_LOGGING_LEVEL)
	...
	log_type = log_level or CONFIG.web_agent_LOGGING_LEVEL

	# Check if handlers are already set up
	if logging.getLogger().hasHandlers() and not force_setup:
		return logging.getLogger('web_agent')
	...
	class webagentFormatter(logging.Formatter):
		...
		def format(self, record):
			...
			if self.log_level > logging.DEBUG and isinstance(record.name, str) and record.name.startswith('web_agent.'):
				...
				if 'Agent' in record.name:
					...
					record.name = 'dom'
				elif record.name.startswith('web_agent.'):
					# For other web_agent modules, use the last part
	...
	if log_type == 'result':
		...
		console.setFormatter(webagentFormatter('%(message)s', log_level))
	else:
		console.setLevel(log_level)  # Keep console at original log level (e.g., INFO)
		console.setFormatter(webagentFormatter('%(levelname)-8s [%(name)s] %(message)s', log_level))
	...
	if debug_log_file:
		...
		debug_handler.setFormatter(webagentFormatter('%(asctime)s - %(levelname)-8s [%(name)s] %(message)s', logging.DEBUG))
	...
	if info_log_file:
		...
		info_handler.setFormatter(webagentFormatter('%(asctime)s - %(levelname)-8s [%(name)s] %(message)s', logging.INFO))
	...
	# Configure web_agent logger
	web_agent_logger = logging.getLogger('web_agent')
	web_agent_logger.propagate = False  # Don't propagate to root logger
	web_agent_logger.addHandler(console)
	for handler in file_handlers:
		web_agent_logger.addHandler(handler)
	web_agent_logger.setLevel(effective_log_level)
	...
	# logger.debug('webagent logging setup complete with level %s', log_type)
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
	...
	for name in ['web_agent.agent', 'web_agent.tools']:
	...
	for name in ['bubus', 'web_agent.browser.session']:
		logger = logging.getLogger(name)

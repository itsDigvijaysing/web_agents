...
from web_agent.agent.views import ActionModel, ActionResult
...
from web_agent.browser.views import BrowserError
from web_agent.dom.service import EnhancedDOMTreeNode
...
from web_agent.tools.utils import get_click_description
...
from web_agent.utils import create_task_with_error_handling, sanitize_surrogates, time_execution_sync
	...
	def __init__(
		...
		async def extract(
			...
			if output_schema is not None:
				try:
					from web_agent.tools.extraction.schema_utils import schema_dict_to_pydantic_model
			...
			from web_agent.dom.markdown_extractor import chunk_markdown_by_structure
...
""".strip()
				...
				try:
					...
					from web_agent.tools.extraction.views import ExtractionResult
...
""".strip()
		...
		async def select_dropdown(params: SelectDropdownOptionAction, browser_session: BrowserSession):
			...
			from web_agent.browser.events import SelectDropdownOptionEvent

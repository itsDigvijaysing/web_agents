...
if '--mcp' in sys.argv:
	...
	os.environ['web_agent_LOGGING_LEVEL'] = 'critical'
	os.environ['web_agent_SETUP_LOGGING'] = 'false'
...
if len(sys.argv) > 1 and sys.argv[1] == 'install':
	...
	if result.returncode == 0:
		...
		print('🚀 Ready to use! Run: uvx web-agent')
	else:
		print('\n❌ Installation failed')
		sys.exit(1)
	sys.exit(0)
...
if 'init' in sys.argv:
	from web_agent.init_cmd import INIT_TEMPLATES
	from web_agent.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'web_agent_{template}.py'
	...
	try:
		...
		click.echo('\nNext steps:')
		click.echo('  1. Install web-agent:')
		click.echo('     uv pip install web-agent')
		click.echo('  2. Set up your API key in .env file or environment:')
		click.echo('     web_agent_API_KEY=your-key')
		click.echo('     (Get your key at https://cloud.web-agent.com/new-api-key)')
...
from web_agent import Agent, Controller
from web_agent.agent.views import AgentSettings
from web_agent.browser import BrowserProfile, BrowserSession
from web_agent.logging_config import addLoggingLevel
from web_agent.telemetry import CLITelemetryEvent, ProductTelemetry
from web_agent.utils import get_web_agent_version
...
try:
	...
	print(
		'⚠️ CLI addon is not installed. Please install it with: `pip install "web-agent[cli]"` and try again.', file=sys.stderr
	)
	sys.exit(1)
...
os.environ['web_agent_LOGGING_LEVEL'] = 'result'

from web_agent.config import CONFIG

# Set USER_DATA_DIR now that CONFIG is imported
USER_DATA_DIR = CONFIG.web_agent_PROFILES_DIR / 'cli'

# Ensure directories exist
CONFIG.web_agent_CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
...
def load_user_config() -> dict[str, Any]:
	...
	history_file = CONFIG.web_agent_CONFIG_DIR / 'command_history.json'
...
class webagentApp(App):
	"""web-agent TUI application."""
	...
	def setup_richlog_logging(self) -> None:
		...
		log_type = os.getenv('web_agent_LOGGING_LEVEL', 'result').lower()

		class webagentFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('web_agent.'):
		...
		if log_type == 'result':
			...
			log_handler.setFormatter(webagentFormatter('%(message)s'))
		else:
			log_handler.setFormatter(webagentFormatter('%(levelname)-8s [%(name)s] %(message)s'))
		...
		# Configure web_agent logger and all its sub-loggers
		web_agent_logger = logging.getLogger('web_agent')
		web_agent_logger.propagate = False  # Don't propagate to root logger
		web_agent_logger.handlers = [log_handler]  # Replace any existing handlers
		web_agent_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['web_agent.Agent', 'web_agent.controller', 'web_agent.agent', 'web_agent.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'web_agent.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('web_agent.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('web_agent.app')
	...
	async def action_quit(self) -> None:
		...
		self.exit()
		print('\nTry running tasks on our cloud: https://web-agent.com')
		...
		with Container(id='main-container'):
			...
			with Container(id='links-panel'):
				with HorizontalGroup(classes='link-row'):
					...
					yield Link('https://web-agent.com', url='https://web-agent.com', classes='link-white link-url')
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/web-agent/awesome-prompts',
						url='https://github.com/web-agent/awesome-prompts',
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/web-agent/web-agent/issues',
						url='https://github.com/web-agent/web-agent/issues',
						classes='link-green link-url',
					)

			# Paths panel
			yield Static(
				f' ⚙️  Settings saved to:              {str(CONFIG.web_agent_CONFIG_FILE.resolve()).replace(str(Path.home()), "~")}\n'
...
async def run_prompt_mode(prompt: str, ctx: click.Context, debug: bool = False):
	"""Run web-agent in non-interactive mode with a single prompt."""
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent web_agent from setting up logging at import time
	os.environ['web_agent_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('web_agent.startup')
	...
	try:
		...
		try:
			from web_agent.logging_config import setup_log_pipes
	...
	logger.debug('Initializing webagentApp instance...')
	try:
		app = webagentApp(config)
...
async def run_auth_command():
	...
	from web_agent.sync.auth import DeviceAuthClient

	print('🔐 web agent Cloud Authentication')
	print('=' * 40)
	...
	if auth_client.is_authenticated:
		...
		frontend_url = CONFIG.web_agent_CLOUD_UI_URL or auth_client.base_url.replace('//api.', '//cloud.')
	...
	try:
		...
		from web_agent.agent.cloud_events import (
		...
		task_event = CreateAgentTaskEvent(
			...
			llm_model='auth-flow',
			task='🔐 Complete authentication and join the web-agent community',
			user_id=auth_client.temp_user_id,
			device_id=auth_client.device_id,
			done_output=None,
		...
		try:
			...
			from web_agent.utils import create_task_with_error_handling
		...
		if success:
				...
				actions=[
					{
						...
						'done': {
							'success': True,
							'text': '⭐ Starred web-agent/web-agent repository! Welcome to the community!',
						},
					}
				],
				next_goal='⭐ Star web-agent GitHub repository to join the community',
				evaluation_previous_goal='Authentication completed successfully',
				memory='User authenticated with web agent Cloud and is now part of the community',
				screenshot_url=None,
				url='https://github.com/web-agent/web-agent',
			)
			...
			completion_event = UpdateAgentTaskEvent(
				...
				user_id=auth_client.temp_user_id,  # Use same temp user_id as task for consistency
				device_id=auth_client.device_id,  # Use consistent device_id
				done_output="🎉 Welcome to web agent! You're now authenticated and part of our community. ⭐ Your future tasks will sync to the cloud automatically.",
				user_feedback_type=None,
				user_comment=None,
				gif_url=None,
			)
			...
			print('   Future web-agent runs will now sync to the cloud.')
		else:
		...
		if task_id and sync_service:
			try:
				from web_agent.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: web_agent_<template>.py)')
...
def main(ctx: click.Context, debug: bool = False, **kwargs):
	...
	Examples:
	  uvx web-agent --template default
	  uvx web-agent --template advanced --output my_script.py
	"""
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	"""Run the main web-agent interface"""

	if kwargs['version']:
		from importlib.metadata import version

		print(version('web-agent'))
		sys.exit(0)
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from web_agent.mcp.server import main as mcp_main
	...
	logger = logging.getLogger('web_agent.startup')
	logger.info('Starting web-agent initialization')
	if debug:
	...
	try:
		config = load_user_config()
		logger.debug(f'User configuration loaded from {CONFIG.web_agent_CONFIG_FILE}')
	except Exception as e:
	...
	try:
		...
		logger.error(f'Error initializing web-agent: {str(e)}', exc_info=debug)
		print(f'\nError launching web-agent: {str(e)}')
		if debug:
			import traceback
...
@main.command()
def auth():
	"""Authenticate with web agent Cloud to sync your runs"""
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'web_agent_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: web_agent_<template>.py)',
)
@click.option(
	'--force',
	'-f',
...
def init(
	...
	"""
	Generate a web-agent template file to get started quickly.
	...
	uvx web-agent init --template advanced --output my_script.py

	\b
	# List available templates
	uvx web-agent init --list
	"""

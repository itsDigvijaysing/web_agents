...
from web_agent.llm import ChatGoogle, ChatOpenAI
from web_agent.llm.messages import AssistantMessage, SystemMessage, UserMessage
...
Only output the country name, no numbers, no punctuation, just the name."""
	...
	if oci_model:
		...
		print('ℹ️  OCI model not available (install with pip install web-agent[oci] and configure credentials)')

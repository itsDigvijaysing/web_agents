...
Browser Actor is a web automation library built on CDP (Chrome DevTools Protocol) that provides low-level browser automation capabilities within the web-agent ecosystem.

## Usage

### Integrated with Browser (Recommended)
```python
from web_agent import Browser  # Alias for BrowserSession
...
```python
from web_agent.actor import Page, Element, Mouse
...
**This is web-agent actor, NOT Playwright or Selenium.** Only use the methods documented above.

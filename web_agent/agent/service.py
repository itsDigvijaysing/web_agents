...
if TYPE_CHECKING:
	from web_agent.skills.views import Skill
...
from web_agent.agent.message_manager.utils import save_conversation
from web_agent.llm.base import BaseChatModel
from web_agent.llm.exceptions import ModelProviderError, ModelRateLimitError
from web_agent.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from web_agent.tokens.service import TokenCost
...
from web_agent import Browser, BrowserProfile, BrowserSession
from web_agent.agent.judge import construct_judge_messages, construct_simple_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from web_agent.agent.gif import create_history_gif
from web_agent.agent.message_manager.service import (
	MessageManager,
)
...
from web_agent.browser.events import _get_timeout
from web_agent.browser.session import DEFAULT_BROWSER_PROFILE
from web_agent.browser.views import BrowserStateSummary
from web_agent.config import CONFIG
from web_agent.dom.views import DOMInteractedElement, MatchLevel
from web_agent.filesystem.file_system import FileSystem
from web_agent.observability import observe, observe_debug
from web_agent.telemetry.service import ProductTelemetry
from web_agent.telemetry.views import AgentTelemetryEvent
...
from web_agent.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_web_agent_version,
	get_web_agent_version,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from web_agent.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from web_agent import Chatwebagent

				llm = Chatwebagent()

		# set flashmode = True if llm is Chatwebagent
		if llm.provider == 'web-agent':
			flash_mode = True
		...
		supports_coordinate_clicking = any(
			pattern in model_name for pattern in ['claude-sonnet-4', 'claude-opus-4', 'gemini-3-pro', 'web-agent/']
		)
		...
		if skill_service is not None:
			...
			from web_agent.skills import SkillService
		...
		self.agent_directory = base_tmp / f'web_agent_agent_{self.id}_{timestamp}'
		...
		self._set_web_agent_version_and_source(source)
		...
		# Check if model is a web-agent fine-tuned model (uses simplified prompts)
		is_web_agent_model = 'web-agent/' in self.llm.model.lower()
		...
		self._message_manager = MessageManager(
			...
			system_message=SystemPrompt(
				...
				is_web_agent_model=is_web_agent_model,
		...
		# Default to ~/.config/webagent/events/{agent_session_id}.jsonl
		# wal_path = CONFIG.web_agent_CONFIG_DIR / 'events' / f'{self.session_id}.jsonl'
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'web_agent.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from web_agent.screenshots.service import ScreenshotService
	...
	def _set_web_agent_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the web-agent package"""
	...
	async def _judge_trace(self) -> JudgementResult | None:
		...
		# Only pass request_type for Chatwebagent (other providers don't support it)
		if self.judge_llm.provider == 'web-agent':
		...
		if self.history.history[-1].result[-1].is_done:
			...
			if judgement:
				...
				if judgement.reached_captcha:
					...
					judge_log += '   👉 🥷 Use web agent Cloud for the most stealth browser infra: https://docs.web-agent.com/customize/browser/remote\n'
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from web_agent.llm.messages import AssistantMessage, UserMessage
	...
	async def get_model_output(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		# Note: Chatwebagent will automatically generate action descriptions from output_format schema
	...
	async def _log_agent_run(self) -> None:
		...
		self.logger.debug(f'🤖 web-agent Library Version {self.version} ({self.source})')

		# Check for latest version and log upgrade message if needed
		if CONFIG.web_agent_VERSION_CHECK:
			latest_version = await check_latest_web_agent_version()
			if latest_version and latest_version != self.version:
				self.logger.info(
					f'📦 Newer version available: {latest_version} (current: {self.version}). Upgrade with: uv add web-agent=={latest_version}'
				)
	...
	def _log_first_step_startup(self) -> None:
		...
		if len(self.history.history) == 0:
			self.logger.info(
				f'Starting a web-agent agent with version {self.version}, with provider={self.llm.provider} and model={self.llm.model}'
			)
	...
	def _log_final_outcome_messages(self) -> None:
		...
		if is_successful is False or is_successful is None:
			...
			self.logger.info('   Open a short issue on GitHub: https://github.com/web-agent/web-agent/issues')
	...
	async def run(
		...
		from web_agent.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from web_agent.agent.views import RerunSummaryAction
		...
		from web_agent.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			try:
				...
				# If structured output fails (e.g., web-agent LLM doesn't support it for this type),
	...
	async def _execute_ai_step(
		...
		"""
		from web_agent.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from web_agent.llm.messages import SystemMessage, UserMessage
		from web_agent.utils import sanitize_surrogates
		...
		try:
			from web_agent.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from web_agent.agent.variable_detector import detect_variables_in_history

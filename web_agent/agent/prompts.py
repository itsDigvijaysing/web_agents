...
from web_agent.dom.views import NodeType, SimplifiedNode
from web_agent.llm.messages import ContentPartImageParam, ContentPartTextParam, ImageURL, SystemMessage, UserMessage
from web_agent.observability import observe_debug
from web_agent.utils import is_new_tab_page, sanitize_surrogates

if TYPE_CHECKING:
	from web_agent.agent.views import AgentStepInfo
...
class SystemPrompt:
	def __init__(
		...
		self.is_web_agent_model = is_web_agent_model
	...
	def _load_prompt_template(self) -> None:
		...
		try:
			...
			# web-agent models use simplified prompts optimized for fine-tuned models
			if self.is_web_agent_model:
				if self.flash_mode:
					template_filename = 'system_prompt_web_agent_flash.md'
				elif self.use_thinking:
					template_filename = 'system_prompt_web_agent.md'
				else:
					template_filename = 'system_prompt_web_agent_no_thinking.md'
			...
			with (
				importlib.resources.files('web_agent.agent.system_prompts')

...
class DownloadsWatchdog(BaseWatchdog):
	...
	async def attach_to_target(self, target_id: TargetID) -> None:
		...
		def download_progress_handler(event: CDPDownloadProgressEvent, session_id: SessionID | None) -> None:
			...
			from web_agent.browser.events import DownloadProgressEvent as DownloadProgressEventInternal
	...
	def _track_download(self, file_path: str, guid: str | None = None) -> None:
		...
		try:
			...
			if path.exists():
				...
				from web_agent.browser.events import FileDownloadedEvent
	...
	async def _handle_cdp_download(
		...
		downloads_dir = (
			Path(
				...
				or f'{tempfile.gettempdir()}/web_agent_downloads.{str(self.browser_session.id)[-4:]}'
			)
			.expanduser()
			.resolve()

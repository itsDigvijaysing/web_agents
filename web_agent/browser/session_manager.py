...
if TYPE_CHECKING:
	from web_agent.browser.session import BrowserSession, CDPSession, Target
...
class SessionManager:
	...
	async def _handle_target_attached(self, event: AttachedToTargetEvent) -> None:
		...
		if target_id not in self._targets:
			from web_agent.browser.session import Target
		...
		from web_agent.browser.session import CDPSession
	...
	async def _handle_target_detached(self, event: DetachedFromTargetEvent) -> None:
		...
		if target_fully_removed:
			if target_type in ('page', 'tab'):
				from web_agent.browser.events import TabClosedEvent
	...
	async def _recover_agent_focus(self, crashed_target_id: TargetID) -> None:
		...
		try:
			...
			if page_targets:
				...
				from web_agent.browser.events import TabCreatedEvent
			...
			if new_session:
				...
				from web_agent.browser.events import AgentFocusChangedEvent
			...
			for _ in range(20):
				...
				if fallback_session:
					...
					from web_agent.browser.events import AgentFocusChangedEvent, TabCreatedEvent

...
# It automatically sets CDP logs to the same level as web_agent logs
from web_agent.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
from web_agent.browser.events import (
...
from web_agent.browser.profile import BrowserProfile, ProxySettings
from web_agent.browser.views import BrowserStateSummary, TabInfo
from web_agent.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from web_agent.observability import observe_debug
from web_agent.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from web_agent.actor.page import Page
	from web_agent.browser.demo_mode import DemoMode
...
class BrowserSession(BaseModel):
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'web_agent.{self}')
		return logging.getLogger(f'web_agent.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from web_agent.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from web_agent.browser.events import SaveStorageStateEvent
				...
				if self.browser_profile.use_cloud or self.browser_profile.cloud_browser_params is not None:
					...
					try:
						...
						raise CloudBrowserAuthError(
							'Authentication failed for cloud browser service. Set web_agent_API_KEY environment variable. You can also create an API key at https://cloud.web-agent.com/new-api-key'
						)
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from web_agent.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from web_agent.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from web_agent.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from web_agent.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from web_agent.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from web_agent.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from web_agent.browser.watchdogs.recording_watchdog import RecordingWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			from web_agent.browser.session_manager import SessionManager
			...
			from web_agent.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from web_agent.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from web_agent.dom.views import NodeType
	...
	async def remove_highlights(self) -> None:
		...
		try:
			...
			script = """
			(function() {
				// Remove all web-agent highlight elements
				const highlights = document.querySelectorAll('[data-web-agent-highlight]');
				console.log('Removing', highlights.length, 'web-agent highlight elements');
				highlights.forEach(el => el.remove());

				// Also remove by ID in case selector missed anything
				const highlightContainer = document.getElementById('web-agent-debug-highlights');
				...
				const orphanedTooltips = document.querySelectorAll('[data-web-agent-highlight="tooltip"]');
	...
	async def highlight_interaction_element(self, node: 'EnhancedDOMTreeNode') -> None:
		...
		try:
			...
			(function() {{
				...
				container.setAttribute('data-web-agent-interaction-highlight', 'true');
	...
	async def highlight_coordinate_click(self, x: int, y: int) -> None:
		...
		try:
			...
			(function() {{
				...
				// Create container
				const container = document.createElement('div');
				container.setAttribute('data-web-agent-coordinate-highlight', 'true');
				container.style.cssText = `
					position: absolute;
					left: ${{x + scrollX}}px;
					top: ${{y + scrollY}}px;
	...
	async def add_highlights(self, selector_map: dict[int, 'EnhancedDOMTreeNode']) -> None:
		...
		try:
			...
			(function() {{
				...
				console.log('=== web-agent HIGHLIGHTING ===');
				...
				const existingContainer = document.getElementById('web-agent-debug-highlights');
				...
				const strayHighlights = document.querySelectorAll('[data-web-agent-highlight]');
				...
				const container = document.createElement('div');
				container.id = 'web-agent-debug-highlights';
				container.setAttribute('data-web-agent-highlight', 'container');
				...
				interactiveElements.forEach((element, index) => {{
					...
					highlight.setAttribute('data-web-agent-highlight', 'element');

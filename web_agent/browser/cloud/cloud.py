"""Cloud browser service integration for web-agent.

This module provides integration with the web-agent cloud browser service.
...
from web_agent.browser.cloud.views import CloudBrowserAuthError, CloudBrowserError, CloudBrowserResponse, CreateBrowserRequest
from web_agent.sync.auth import CloudAuthConfig

logger = logging.getLogger(__name__)


class CloudBrowserClient:
	"""Client for web-agent cloud browser service."""

	def __init__(self, api_base_url: str = 'https://api.web-agent.com'):
	...
	async def create_browser(
		...
		"""Create a new cloud browser instance. For full docs refer to https://docs.cloud.web-agent.com/api-reference/v-2-api-current/browsers/create-browser-session-browsers-post
		...
		api_token = os.getenv('web_agent_API_KEY')
		...
		if not api_token:
			raise CloudBrowserAuthError(
				'No authentication token found. Please set web_agent_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.web-agent.com/new-api-key'
			)

		headers = {'X-web-agent-API-Key': api_token, 'Content-Type': 'application/json', **(extra_headers or {})}
		...
		try:
			...
			if response.status_code == 401:
				raise CloudBrowserAuthError(
					'Authentication failed. Please make sure you have set web_agent_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.web-agent.com/new-api-key'
				)
			elif response.status_code == 403:
				raise CloudBrowserAuthError('Access forbidden. Please check your web-agent cloud subscription status.')
	...
	async def stop_browser(
		...
		try:
			...
			if response.status_code == 401:
				raise CloudBrowserAuthError(
					'Authentication failed. Please make sure you have set the web_agent_API_KEY environment variable to authenticate with the cloud service.'
				)

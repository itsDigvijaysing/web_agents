...
from web_agent.browser.cloud.views import CloudBrowserParams
from web_agent.config import CONFIG
from web_agent.utils import _log_pretty_path, logger


def _get_enable_default_extensions_default() -> bool:
	"""Get the default value for enable_default_extensions from env var or True."""
	env_val = os.getenv('web_agent_DISABLE_EXTENSIONS')
...
# Using constants from central location in web_agent.config
webagent_DEFAULT_CHANNEL = BrowserChannel.CHROMIUM
...
class BrowserLaunchArgs(BaseModel):
	...
	def set_default_downloads_path(self) -> Self:
		...
		if self.downloads_path is None:
			...
			downloads_path = Path(f'/tmp/web-agent-downloads-{unique_id}')
...
class BrowserLaunchPersistentContextArgs(BrowserLaunchArgs, BrowserContextArgs):
	...
	def validate_user_data_dir(cls, v: str | Path | None) -> str | Path:
		...
		if v is None:
			return tempfile.mkdtemp(prefix='web-agent-user-data-dir-')
...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	use_cloud: bool = Field(
		default=False,
		description='Use web-agent cloud browser service instead of local browser',
	)
	...
	# 	default_factory=lambda: Path('~/.config/webagent/cache/extensions').expanduser(),
	...
	def warn_user_data_dir_non_default_version(self) -> Self:
		...
		is_not_using_default_chromium = self.executable_path or self.channel not in (webagent_DEFAULT_CHANNEL, None)
		if self.user_data_dir == CONFIG.web_agent_DEFAULT_USER_DATA_DIR and is_not_using_default_chromium:
			alternate_name = (
			...
			logger.warning(
				f'⚠️ {self} Changing user_data_dir= {_log_pretty_path(self.user_data_dir)} ➡️ .../default-{alternate_name} to avoid {alternate_name.upper()} corruping default profile created by {webagent_DEFAULT_CHANNEL.name}'
			)
			self.user_data_dir = CONFIG.web_agent_DEFAULT_USER_DATA_DIR.parent / f'default-{alternate_name}'
		return self
	...
	def _copy_profile(self) -> None:
		...
		if 'web-agent-user-data-dir-' in user_data_str.lower():
		...
		temp_dir = tempfile.mkdtemp(prefix='web-agent-user-data-dir-')
	...
	def _ensure_default_extensions_downloaded(self) -> list[str]:
		...
		cache_dir = CONFIG.web_agent_EXTENSIONS_DIR

...
_DEMO_PANEL_SCRIPT = r"""(function () {
  ...
  const SESSION_ID = '__web_agent_SESSION_ID_PLACEHOLDER__';
  const EXCLUDE_ATTR = 'data-web-agent-exclude-' + SESSION_ID;
  const PANEL_ID = 'web-agent-demo-panel';
  const STYLE_ID = 'web-agent-demo-panel-style';
  const STORAGE_KEY = '__webagentDemoLogs__';
  const STORAGE_HTML_KEY = '__webagentDemoLogsHTML__';
  const PANEL_STATE_KEY = '__webagentDemoPanelState__';
  const TOGGLE_BUTTON_ID = 'web-agent-demo-toggle';
  const MAX_MESSAGES = 100;
  const EXPANDED_IDS_KEY = '__webagentExpandedEntries__';
  const LEVEL_ICONS = {
    info: 'ℹ️',
    action: '▶️',
    thought: '💭',
  ...
  if (window.__webagentDemoPanelLoaded) {
  ...
  window.__webagentDemoPanelLoaded = true;
  ...
  function initializePanel() {
    console.log('web-agent demo panel initialized with session ID:', SESSION_ID);
    addStyles();
    ...
    if (savedWidth) {
      document.documentElement.style.setProperty('--web-agent-demo-panel-width', `${savedWidth}px`);
    }
  ...
  function addStyles() {
    ...
    style.textContent = `
      #${PANEL_ID} {
        ...
        top: 0;
        right: 0;
        width: var(--web-agent-demo-panel-width, 340px);
      ...
      #${PANEL_ID} .web-agent-demo-header {
      ...
      #${PANEL_ID} .web-agent-demo-header h1 {
        font-size: 15px;
      ...
      #${PANEL_ID} .web-agent-logo img {
        height: 36px;
      }

      #${PANEL_ID} .web-agent-header-actions {
        margin-left: auto;
        display: flex;
        align-items: center;
        gap: 8px;
      ...
      #${PANEL_ID} .web-agent-close-btn:hover {
        background: rgba(255, 255, 255, 0.15);
        border-color: rgba(255, 255, 255, 0.35);
      }

      #${PANEL_ID} .web-agent-demo-body {
        flex: 1;
        overflow-y: auto;
      ...
      #${PANEL_ID} .web-agent-demo-body::-webkit-scrollbar {
        width: 8px;
      }

      #${PANEL_ID} .web-agent-demo-body::-webkit-scrollbar-thumb {
        background: rgba(255, 255, 255, 0.25);
        border-radius: 999px;
      }

      .web-agent-demo-entry {
        display: flex;
        gap: 12px;
        padding: 10px 18px;
        border-left: 2px solid transparent;
        border-bottom: 1px solid rgba(255, 255, 255, 0.04);
        animation: web-agent-fade-in 0.25s ease;
        background: #000000;
      }

      .web-agent-demo-entry:last-child {
        border-bottom-color: transparent;
      }

      .web-agent-entry-icon {
        font-size: 16px;
        line-height: 1.2;
        width: 20px;
      }

      .web-agent-entry-content {
        flex: 1;
        min-width: 0;
      }

      .web-agent-entry-meta {
        font-size: 11px;
        letter-spacing: 0.08em;
        text-transform: uppercase;
        color: white;
      ...
      .web-agent-markdown-content {
        margin: 0;
        line-height: 1.5;
      }

      .web-agent-markdown-content p {
        margin: 0 0 8px 0;
      }

      .web-agent-markdown-content p:last-child {
        margin-bottom: 0;
      }

      .web-agent-markdown-content h1,
      .web-agent-markdown-content h2,
      .web-agent-markdown-content h3 {
        margin: 8px 0 4px 0;
        font-weight: 600;
        color: #f8f9ff;
      }
      ...
      .web-agent-markdown-content code {
      ...
      .web-agent-markdown-content pre {
      ...
      .web-agent-markdown-content pre code {
      ...
      .web-agent-markdown-content ul,
      .web-agent-markdown-content ol {
        margin: 4px 0 4px 16px;
        padding: 0;
      }

      .web-agent-markdown-content li {
        margin: 2px 0;
      }

      .web-agent-markdown-content a {
        color: #60a5fa;
        text-decoration: underline;
      }

      .web-agent-markdown-content a:hover {
        color: #93c5fd;
      }

      .web-agent-markdown-content strong {
        font-weight: 600;
        color: #f8f9ff;
      }

      .web-agent-markdown-content em {
        font-style: italic;
      }

      .web-agent-demo-entry:not(.expanded) .web-agent-markdown-content {
        max-height: 120px;
        overflow: hidden;
        mask-image: linear-gradient(to bottom, rgba(0,0,0,1), rgba(0,0,0,0));
      }

      .web-agent-entry-toggle {
      ...
      .web-agent-demo-entry.level-info { border-left-color: #60a5fa; }
      .web-agent-demo-entry.level-action { border-left-color: #34d399; }
      .web-agent-demo-entry.level-thought { border-left-color: #f97316; }
      .web-agent-demo-entry.level-warning { border-left-color: #fbbf24; }
      .web-agent-demo-entry.level-success { border-left-color: #22c55e; }
      .web-agent-demo-entry.level-error { border-left-color: #f87171; }

      @keyframes web-agent-fade-in {
  ...
  function buildPanel() {
    ...
    panel.setAttribute('aria-label', 'web-agent demo panel');
    panel.setAttribute(EXCLUDE_ATTR, 'true');

    const header = document.createElement('header');
    header.className = 'web-agent-demo-header';
    const title = document.createElement('div');
    title.className = 'web-agent-logo';
    const logo = document.createElement('img');
    logo.src = 'https://raw.githubusercontent.com/web-agent/web-agent/main/static/web-agent-dark.png';
    logo.alt = 'web-agent';
    logo.loading = 'lazy';
    title.appendChild(logo);
    const actions = document.createElement('div');
    actions.className = 'web-agent-header-actions';
    const closeBtn = document.createElement('button');
    closeBtn.type = 'button';
    closeBtn.className = 'web-agent-close-btn';
    ...
    body.className = 'web-agent-demo-body';
  ...
  function adjustLayout() {
    ...
    document.documentElement.style.setProperty('--web-agent-demo-panel-width', `${width}px`);
    if (state.isOpen) {
  ...
  function computePanelWidth() {
    ...
    try {
      sessionStorage.setItem('__webagentPanelWidth__', String(width));
    } catch {
      // fallthrough
    }
    return width;
  ...
  function loadPanelWidth() {
    try {
      const saved = sessionStorage.getItem('__webagentPanelWidth__');
  ...
  function hydrateFromStoredMarkup() {
    ...
    try {
      ...
      if (html) {
        ...
        for (const entryNode of state.list.querySelectorAll('.web-agent-demo-entry')) {
          const toggle = entryNode.querySelector('.web-agent-entry-toggle');
          if (toggle) {
  ...
  function createEntryNode(entry) {
    ...
    if (messageText.length > 160) {
      ...
      toggle.type = 'button';
      toggle.className = 'web-agent-entry-toggle';
  ...
  function applyPersistedExpansion(node) {
    ...
    try {
      ...
      if (id && expanded.has(id)) {
        ...
        const toggle = node.querySelector('.web-agent-entry-toggle');
        if (toggle) {
  ...
  const boot = () => {
    if (window.__webagentDemoPanelBootstrapped) {
      return;
    }
    ...
    const start = () => {
      ...
      window.__webagentDemoPanelBootstrapped = true;
      initializePanel();
    };
  ...
  window.addEventListener('web-agent-log', handleLogEvent);
})();
"""
...
class DemoMode:
	...
	def _load_script(self) -> str:
		...
		script_with_session_id = self._script_source.replace('__web_agent_SESSION_ID_PLACEHOLDER__', session_id)
...
(() => {{
	...
	const event = new CustomEvent('web-agent-log', {{ detail }});

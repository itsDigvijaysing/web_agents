"""
OAuth2 Device Authorization Grant flow client for web-agent.
"""
...
def get_or_create_device_id() -> str:
	...
	device_id_path = CONFIG.web_agent_CONFIG_DIR / 'device_id'
	...
	CONFIG.web_agent_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
...
class CloudAuthConfig(BaseModel):
	...
	def load_from_file(cls) -> 'CloudAuthConfig':
		...
		config_path = CONFIG.web_agent_CONFIG_DIR / 'cloud_auth.json'
...
class DeviceAuthClient:
	...
	def __init__(self, base_url: str | None = None, http_client: httpx.AsyncClient | None = None):
		...
		self.base_url = base_url or CONFIG.web_agent_CLOUD_API_URL
	...
	async def authenticate(
		...
		try:
			...
			frontend_url = CONFIG.web_agent_CLOUD_UI_URL or self.base_url.replace('//api.', '//cloud.')
			...
			if show_instructions and CONFIG.web_agent_CLOUD_SYNC:
				logger.info('─' * max(terminal_width - 40, 20))
				logger.info('🌐  View the details of this run in web agent Cloud:')
			...
			if token_data and token_data.get('access_token'):
				...
				if show_instructions:
					logger.debug('✅  Authentication successful! Cloud sync is now enabled with your web-agent account.')
			...
			if e.response.status_code == 404:
				logger.warning(
					'Cloud sync authentication endpoint not found (404). Check your web_agent_CLOUD_API_URL setting.'
				)
			else:
		...
		if show_instructions:
			logger.debug(f'❌ Sync authentication failed or timed out with {CONFIG.web_agent_CLOUD_API_URL}')

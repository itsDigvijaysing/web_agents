...
This module handles the conversion between web-agent message formats
...
class OCIRawMessageSerializer:
	"""
	Serializer for converting between web-agent message types and OCI Raw API message formats.
	...
	def serialize_messages(messages: list[BaseMessage]) -> list[Message]:
		"""
		Serialize a list of web-agent messages to OCI Raw API Message objects.

		Args:
		    messages: List of web-agent messages
				...
				if isinstance(content, str):
					...
					for part in content:
						if part.type == 'text':
							...
							# Note: This is currently unreachable in web-agent but kept for completeness
							image_content = OCIRawMessageSerializer._create_image_content(part)
							contents.append(image_content)

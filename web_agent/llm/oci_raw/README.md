...
from web_agent.llm import ChatOCIRaw
...
**Important**: Only models that support tool calling/function calling are compatible with web-agent. Tool calling is essential for web-agent as the agent needs to call browser automation functions (click, type, scroll, etc.) to interact with web pages.

According to Oracle's documentation, tool calling functionality is available exclusively through the API and is not supported for browser-based use. However, when using web-agent with OCI models through this integration, the tool calling happens at the application level (not browser-based), making it compatible.

### Image Support Models

Several OCI models support image processing capabilities, which are useful when web-agent needs to analyze webpage screenshots:
...
These vision-enabled models are particularly useful for web-agent tasks that require understanding webpage content through screenshots, such as:

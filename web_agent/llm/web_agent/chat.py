"""
Chatwebagent - Client for web-agent cloud API

This wraps the BaseChatModel protocol and sends requests to the web-agent cloud API
...
from web_agent.observability import observe
...
class Chatwebagent(BaseChatModel):
	"""
	Client for web-agent cloud API.

	This sends requests to the web-agent cloud API which uses optimized models
	and prompts for browser automation tasks.

	Usage:
		agent = Agent(
			task="Find the number of stars of the web-agent repo",
			llm=Chatwebagent(model='bu-latest'),
		)
	"""
	...
	def __init__(
		...
		**kwargs,
		...
		"""
		Initialize Chatwebagent client.

		Args:
			model: Model name to use. Options:
				- 'bu-latest' or 'bu-1-0': Default model
				- 'bu-2-0': Latest premium model
				- 'web-agent/bu-30b-a3b-preview': web agent Open Source Model
			api_key: API key for web-agent cloud. Defaults to web_agent_API_KEY env var.
			base_url: Base URL for the API. Defaults to web_agent_LLM_URL env var or production URL.
		...
		"""
		# Validate model name - allow bu-* and web-agent/* patterns
		valid_models = ['bu-latest', 'bu-1-0', 'bu-2-0']
		is_valid = model in valid_models or model.startswith('web-agent/')
		if not is_valid:
			raise ValueError(f"Invalid model: '{model}'. Must be one of {valid_models} or start with 'web-agent/'")
		...
		self.fast = False
		self.api_key = api_key or os.getenv('web_agent_API_KEY')
		self.base_url = base_url or os.getenv('web_agent_LLM_URL', 'https://llm.api.web-agent.com')
		...
		if not self.api_key:
			raise ValueError(
				'You need to set the web_agent_API_KEY environment variable. '
				'Get your key at https://cloud.web-agent.com/new-api-key'
			)
	...
	@observe(name='chat_web_agent_ainvoke')
	async def ainvoke(
		self,
		...
		**kwargs: Any,
		...
		"""
		Send request to web-agent cloud API.
		...
		for attempt in range(self.max_retries):
			try:
				...
				raise ValueError(f'Failed to connect to web-agent API after {self.max_retries} attempts: {e}')

			except Exception as e:
				raise ValueError(f'Failed to connect to web-agent API: {e}')
		else:
		...
		if 'usage' in result and result['usage'] is not None:
			from web_agent.llm.views import ChatInvokeUsage

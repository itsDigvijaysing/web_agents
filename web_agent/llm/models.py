...
def get_llm_by_name(model_name: str):
	...
	# web agent Models
		...
		return Chatwebagent(model=model, api_key=api_key)
...
def __getattr__(name: str) -> 'BaseChatModel':
	...
	if name == 'ChatOpenAI':
		...
		if not OCI_AVAILABLE:
			raise ImportError('OCI integration not available. Install with: pip install "web-agent[oci]"')
		return ChatOCIRaw  # type: ignore
	elif name == 'ChatCerebras':
		return ChatCerebras  # type: ignore
	elif name == 'Chatwebagent':
		return Chatwebagent  # type: ignore
...
__all__ += [
	...
	# web agent instances - created on demand
	'bu_latest',
	'bu_1_0',
	'bu_2_0',
]
...
# so this module can be imported without web-agent[oci] installed.

"""MCP (Model Context Protocol) client integration for web-agent.

This module provides integration between external MCP servers and web-agent's action registry.
MCP tools are dynamically discovered and registered as web-agent actions.
...
Example usage:
    ...
    # Register all MCP tools as web-agent actions
...
from web_agent.telemetry import MCPClientTelemetryEvent, ProductTelemetry
from web_agent.tools.registry.service import Registry
...
class MCPClient:
	"""Client for connecting to MCP servers and exposing their tools as web-agent actions."""
	...
	async def register_to_tools(
		...
		"""Register MCP tools as actions in the web-agent tools.

		Args:
			tools: web-agent tools to register actions to
		...
		logger.info(f"✅ Registered {len(self._registered_actions)} MCP tools from '{self.server_name}' as web-agent actions")

	def _register_tool_as_action(self, registry: Registry, action_name: str, tool: Any) -> None:
		"""Register a single MCP tool as a web-agent action.

		Args:
			registry: web-agent registry to register action to
		...
		# Register the action with web-agent

"""MCP (Model Context Protocol) tool wrapper for web-agent.

This module provides integration between MCP tools and web-agent's action registry system.
...
class MCPToolWrapper:
	"""Wrapper to integrate MCP tools as web-agent actions."""

	def __init__(self, registry: Registry, mcp_command: str, mcp_args: list[str] | None = None):
		"""Initialize MCP tool wrapper.

		Args:
			registry: web-agent action registry to register MCP tools
	...
	def _register_tool_as_action(self, tool_name: str, tool: Tool):
		"""Register an MCP tool as a web-agent action.
...
async def register_mcp_tools(registry: Registry, mcp_command: str, mcp_args: list[str] | None = None) -> MCPToolWrapper:
	"""Register MCP tools with a web-agent registry.

	Args:
		registry: web-agent action registry
	...
	Example:
		```python
	        ...
	        from web_agent.mcp.tools import register_mcp_tools
	        ...
	        # Now all MCP tools are available as web-agent actions
		```
	"""

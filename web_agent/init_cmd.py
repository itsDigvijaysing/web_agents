"""
Standalone init command for web-agent template generation.

This module provides a minimal command-line interface for generating
web-agent templates without requiring heavy TUI dependencies.
"""
...
# InquirerPy style for template selection (web-agent orange theme)
...
def main(
	...
	if _write_init_file(output_path, content, force):
		...
		if 'next_steps' in INIT_TEMPLATES[template]:
			...
			next_steps.append('3. Install web-agent:\n', style='bold')
			next_steps.append('   uv add web-agent\n\n', style='dim')
			next_steps.append('4. Set up your API key in .env file or environment:\n', style='bold')
			next_steps.append('   web_agent_API_KEY=your-key\n', style='dim')
			next_steps.append(
				'   (Get your key at https://cloud.web-agent.com/dashboard/settings?tab=api-keys&new)\n\n',
				style='dim italic',
			)

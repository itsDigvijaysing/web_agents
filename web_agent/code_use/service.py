...
from web_agent.tokens.views import UsageSummary
from web_agent.tools.service import CodeAgentTools, Tools
	...
	def __init__(
		...
		**kwargs,
		...
		Args:
			...
			llm: Optional Chatwebagent LLM instance (will create default if not provided)
		...
		if llm is None:
			try:
				...
				llm = Chatwebagent()
				logger.debug('CodeAgent using Chatwebagent')
			except Exception as e:
				raise RuntimeError(f'Failed to initialize CodeAgent LLM: {e}')

		if 'Chatwebagent' not in llm.__class__.__name__:
			raise ValueError('This agent works only with Chatwebagent.')
		...
		base_tmp = Path('/tmp')
		self.agent_directory = base_tmp / f'web_agent_code_agent_{self.id}_{timestamp}'
		...
		self.version = get_web_agent_version()
		try:

...
async def handle(action: str, session: SessionInfo, params: dict[str, Any]) -> Any:
	...
	if action == 'open':
		...
		if bs.browser_profile.use_cloud and bs.cdp_url:
			...
			result['live_url'] = f'https://live.web-agent.com/?wss={quote(bs.cdp_url, safe="")}'
		return result
		...
		from web_agent.browser.events import ClickElementEvent, TypeTextEvent
		...
		# Return the same LLM representation that web-agent agents see
		...
		from web_agent.browser.events import SendKeysEvent
		...
		return {'query': query, 'error': 'extract requires agent mode - use: web-agent run "extract ..."'}

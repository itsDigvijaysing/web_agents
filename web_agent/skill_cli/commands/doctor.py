...
Validates that web-agent is properly installed and all dependencies
...
def _check_package() -> dict[str, Any]:
	"""Check if web-agent is installed."""
	try:
		import web_agent

		version = getattr(web_agent, '__version__', 'unknown')
		return {
			'status': 'ok',
			'message': f'web-agent {version}',
		}
	except ImportError:
		return {
			'status': 'error',
			'message': 'web-agent not installed',
			'fix': 'pip install web-agent',
		}
...
def _check_api_key_config() -> dict[str, Any]:
	...
	if status['available']:
		...
		return {
			...
			'note': 'Required for remote browser. Get one at https://web-agent.com/new-api-key',
		}

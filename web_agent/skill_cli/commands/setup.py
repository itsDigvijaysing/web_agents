"""Setup command - configure web-agent for first-time use.
...
async def run_checks(mode: Literal['local', 'remote', 'full']) -> dict[str, Any]:
	...
	try:
		...
		checks['web_agent_package'] = {
			'status': 'ok',
			'message': f'web-agent {web_agent.__version__}'
			if hasattr(web_agent, '__version__')
			else 'web-agent installed',
		}
	except ImportError:
		checks['web_agent_package'] = {
			'status': 'error',
			'message': 'web-agent not installed',
		}
	...
	if mode in ('remote', 'full'):
		from web_agent.skill_cli.api_key import check_api_key
	...
	if mode in ('remote', 'full'):
		from web_agent.skill_cli.tunnel import get_tunnel_manager
...
async def execute_actions(
	...
	for action in actions:
		...
		if action_type == 'install_browser':
			...
			from web_agent.skill_cli.api_key import save_api_key
			...
			if not json_output:
				...
				print('   Set via: export web_agent_API_KEY=your_key')
				print('   Or: web-agent setup --api-key <key>')
...
async def validate_setup(
	...
	try:
		...
		results['web_agent_import'] = 'ok'
	except ImportError:
		results['web_agent_import'] = 'failed'

...
from web_agent.skill_cli.commands.utils import get_sdk_client
...
def _print_usage() -> None:
	...
	print('Usage: web-agent [-b real|remote] profile <command>')
	print()
	print('Commands:')
...
def _list_cloud_profiles(args: argparse.Namespace) -> int:
	"""List cloud profiles."""
	from web_agent.skill_cli.api_key import APIKeyRequired
...
def _list_profile_cookies(args: argparse.Namespace) -> int:
	...
	if getattr(args, 'json', False):
		...
		print(f'  web-agent profile sync --from "{profile_id}" --domain <domain>')
...
def _handle_sync(args: argparse.Namespace) -> int:
	...
	if not from_profile:
		...
		print('  web-agent profile sync --from "Default"')
		print('  web-agent profile sync --from "Profile 1"')
		return 1
	...
	async def sync_cookies():
		...
		try:
			...
			cookies_file = Path(tempfile.gettempdir()) / f'web-agent-sync-{cloud_profile_id}.json'
	...
	log(f'  web-agent -b remote --profile {cloud_profile_id} open <url>')

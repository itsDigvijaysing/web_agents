"""API key management for web-agent CLI."""
...
def get_config_path() -> Path:
	...
	return base / 'web-agent' / 'config.json'
...
def require_api_key(feature: str = 'this feature') -> str:
	...
	Checks in order:
	1. web_agent_API_KEY environment variable
	2. Config file (~/.config/web-agent/config.json)
	3. Interactive prompt (if TTY)
	4. Raises APIKeyRequired with helpful message
	"""
	# 1. Check environment
	key = os.environ.get('web_agent_API_KEY')
	if key:
		return key
	...
	raise APIKeyRequired(
		f"""
╭─────────────────────────────────────────────────────────────╮
│  🔑 web-agent API Key Required                            │
│                                                             │
│  {feature} requires an API key.                             │
│                                                             │
│  Get yours at: https://web-agent.com/new-api-key            │
│                                                             │
│  Then set it via:                                           │
│    export web_agent_API_KEY=your_key_here                 │
│                                                             │

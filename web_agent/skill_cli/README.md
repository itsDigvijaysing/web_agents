...
```powershell
& "C:\Program Files\Git\bin\bash.exe" -c 'curl -fsSL https://web-agent.com/cli/install.sh | bash'
```
...
curl -fsSL https://web-agent.com/cli/install.sh | bash -s -- --api-key bu_xxx  # With API key
```

### Post-Install
```bash
web-agent doctor   # Validate installation
web-agent setup    # Run setup wizard (optional)
web-agent setup --mode local|remote|full  # Non-interactive setup
web-agent setup --api-key bu_xxx --yes    # With API key, skip prompts
```
...
export web_agent_API_KEY=your_key  # or $env:web_agent_API_KEY on Windows
...
web-agent input 3 "john@example.com"

# Take a screenshot
web-agent screenshot output.png
...
web-agent --headed open https://example.com

# Use your real Chrome (with existing logins/cookies)
web-agent --browser real open https://gmail.com

# Cloud browser (requires web_agent_API_KEY)
web-agent --browser remote open https://example.com
```
...
web-agent python "print(browser.url)"  # Access browser
...
### Local Mode
```bash
web-agent run "Fill the contact form with test data"
web-agent run "Extract all product prices" --max-steps 50
web-agent run "task" --llm gpt-4o   # Specify LLM model
```
...
### Remote Mode (Cloud)
```bash
web-agent -b remote run "Search for AI news"              # US proxy default
web-agent -b remote run "task" --llm gpt-4o               # Specify LLM
web-agent -b remote run "task" --proxy-country gb         # UK proxy
web-agent -b remote run "task" --session-id <id>          # Reuse session
web-agent -b remote run "task" --no-wait                  # Async (returns task ID)
web-agent -b remote run "task" --wait                     # Wait for completion
web-agent -b remote run "task" --stream                   # Stream output
web-agent -b remote run "task" --flash                    # Fast mode
web-agent -b remote run "task" --keep-alive               # Keep session alive
web-agent -b remote run "task" --thinking                 # Extended reasoning
web-agent -b remote run "task" --vision                   # Enable vision (default)
web-agent -b remote run "task" --no-vision                # Disable vision
web-agent -b remote run "task" --profile <id>             # Use cloud profile

# Task configuration
web-agent -b remote run "task" --start-url https://example.com  # Start from URL
...
web-agent -b remote run "task" --structured-output '{"type":"object"}'  # JSON schema
web-agent -b remote run "task" --judge                          # Enable judge mode
web-agent -b remote run "task" --judge-ground-truth "answer"    # Expected answer
```

Requires `web_agent_API_KEY`.
...
web-agent tunnel 3000                    # → https://abc.trycloudflare.com
web-agent -b remote open https://abc.trycloudflare.com
```
...
```bash
web-agent open https://news.ycombinator.com
web-agent eval "Array.from(document.querySelectorAll('.titleline a')).slice(0,5).map(a => a.textContent)"
```
...
web-agent --session work state
web-agent --session personal state
web-agent close --all
```
...
# Check task status
web-agent task status task-123

# Run another task in same session (preserves login)
web-agent -b remote run "Go to settings" --session-id sess-456
```
...
4. Server auto-starts when needed, stops with `web-agent server stop`
...
# Or run via Git Bash
& "C:\Program Files\Git\bin\bash.exe" -c 'web-agent --help'
```
...
```powershell
taskkill /IM python.exe /F
Remove-Item -Recurse -Force "$env:USERPROFILE\.web-agent-env"

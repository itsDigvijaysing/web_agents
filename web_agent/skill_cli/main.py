#!/usr/bin/env python3
"""Fast CLI for web-agent. STDLIB ONLY - must start in <50ms.

This is the main entry point for the web-agent CLI. It uses only stdlib
...
# not just anywhere in argv (prevents hijacking: web-agent run "init something")
...
def get_socket_path(session: str) -> str:
	...
	return str(Path(tempfile.gettempdir()) / f'web-agent-{session}.sock')


def get_pid_path(session: str) -> Path:
	"""Get PID file path for session."""
	return Path(tempfile.gettempdir()) / f'web-agent-{session}.pid'
...
def get_session_metadata_path(session: str) -> Path:
	...
	return Path(tempfile.gettempdir()) / f'web-agent-{session}.meta'
...
def ensure_server(session: str, browser: str, headed: bool, profile: str | None, api_key: str | None) -> bool:
	...
	from web_agent.skill_cli.utils import is_session_locked, kill_orphaned_server
					...
					if existing_mode != browser:
						...
						if browser == 'remote' and existing_mode != 'remote':
							print(
								...
								f'Options:\n'
								f'  1. Close and restart: web-agent close && web-agent --browser remote open <url>\n'
								f'  2. Use different session: web-agent --browser remote --session other <command>\n'
								f'  3. Use existing local browser: web-agent --browser {existing_mode} <command>',
								file=sys.stderr,
							)
							sys.exit(1)
...
def build_parser() -> argparse.ArgumentParser:
	...
	from web_agent.skill_cli.install_config import get_available_modes, get_default_mode
  ...
  web-agent run "Fill the form"               # Uses local browser + your API keys
  web-agent run "task" --llm gpt-4o           # Specify model (requires API key)
...
Remote Mode (--browser remote):
  web-agent -b remote run "task"              # Cloud execution (US proxy default)
  web-agent -b remote run "task" --llm gpt-4o # Specify cloud model
  web-agent -b remote --profile <id> run "task"  # Use cloud profile
  web-agent -b remote run "task" --proxy-country gb     # UK proxy
  web-agent -b remote run "task" --session-id <id>      # Reuse session
  web-agent -b remote run "task" --wait       # Wait for completion

Task Management:
  web-agent task list                         # List recent cloud tasks
  web-agent task status <task-id>             # Check task status
  web-agent task stop <task-id>               # Stop running task""")
		else:
			# Remote-only install
			epilog_parts.append("""
Examples:
  web-agent run "task"                        # Cloud execution (US proxy default)
  web-agent run "task" --llm gpt-4o           # Specify model
  web-agent --profile <id> run "task"         # Use cloud profile
  web-agent run "task" --proxy-country gb     # UK proxy
  web-agent run "task" --session-id <id>      # Reuse existing session
...
Task Management:
  ...
  web-agent task stop <task-id>               # Stop running task""")
	...
	epilog_parts.append("""
Setup:
  web-agent install                           # Install Chromium browser
  web-agent init                              # Generate template file""")

	parser = argparse.ArgumentParser(
		prog='web-agent',
		description='Browser automation CLI for web-agent',
	...
	parser.add_argument('--api-key', help='web-agent API key')
	...
	# init
	p = subparsers.add_parser('init', help='Generate web-agent template file')
	...
	# setup
	p = subparsers.add_parser('setup', help='Configure web-agent for first-time use')
	p.add_argument('--mode', choices=['local', 'remote', 'full'], default='local', help='Setup mode (local/remote/full)')
	p.add_argument('--api-key', help='web-agent API key')
	p.add_argument('--yes', '-y', action='store_true', help='Skip interactive prompts')

	# doctor
	subparsers.add_parser('doctor', help='Check web-agent installation and dependencies')
...
def handle_server_command(args: argparse.Namespace) -> int:
	...
	if args.server_command == 'status':
		...
		log_path = Path(tempfile.gettempdir()) / f'web-agent-{args.session}.log'
		if log_path.exists():
...
def main() -> int:
	...
	if args.command == 'profile':
		from web_agent.skill_cli.commands.profile import handle_profile_command

		return handle_profile_command(args)

	# Handle sessions list - find all running sessions
	if args.command == 'sessions':
		from web_agent.skill_cli.utils import find_all_sessions
	...
	if args.command == 'setup':
		from web_agent.skill_cli.commands import setup
		...
		if args.json:
			...
			if result.get('status') == 'success':
				print('\n✓ Setup complete!')
				print(f'\nMode: {result["mode"]}')
				print('Next: web-agent open https://example.com')
		return 0

	# Handle doctor command
	if args.command == 'doctor':
		from web_agent.skill_cli.commands import doctor
	...
	if args.command == 'tunnel':
		from web_agent.skill_cli import tunnel
		...
		if pos == 'list':
			...
			if getattr(args, 'all', False):
				...
				print('Usage: web-agent tunnel stop <port> | --all', file=sys.stderr)
				return 1
			...
			print('Usage: web-agent tunnel <port> | list | stop <port>', file=sys.stderr)
			return 0
	...
	from web_agent.skill_cli.install_config import get_mode_unavailable_error, is_mode_available
	...
	if args.api_key:
		os.environ['web_agent_API_KEY'] = args.api_key
	...
	if args.browser == 'remote':
		...
		try:
			...
			os.environ['web_agent_API_KEY'] = api_key

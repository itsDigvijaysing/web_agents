...
modes (chromium, real, remote) are available based on how web-agent was installed.

Config file: ~/.web-agent/install-config.json
...
CONFIG_PATH = Path.home() / '.web-agent' / 'install-config.json'
...
def get_mode_unavailable_error(mode: str) -> str:
	...
	return (
		...
		f'  curl -fsSL https://web-agent.com/cli/install.sh | bash -s -- {install_flag}'
	)

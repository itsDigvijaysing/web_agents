...
#   curl -fsSL https://web-agent.com/cli/install.sh | bash -s -- --full
#   curl -fsSL https://web-agent.com/cli/install.sh | bash -s -- --remote-only
#   curl -fsSL https://web-agent.com/cli/install.sh | bash -s -- --local-only
#
#   # With API key
#   curl -fsSL https://web-agent.com/cli/install.sh | bash -s -- --remote-only --api-key bu_xxx
#
# For development testing:
#   curl -fsSL <raw-url> | web_agent_BRANCH=<branch-name> bash
#
# =============================================================================
...
#   & "C:\Program Files\Git\bin\bash.exe" -c 'curl -fsSL https://web-agent.com/cli/install.sh | bash -s -- --full'
#
...
#      & "C:\Program Files\Git\bin\bash.exe" -c 'web-agent open https://example.com'
#
...
#      & "$env:USERPROFILE\.web-agent-env\Scripts\python.exe" -m web_agent.skill_cli.server --session default --browser chromium
	...
	while [[ $# -gt 0 ]]; do
		case $1 in
			...
			--help|-h)
				...
				echo "  --api-key KEY     Set web-agent API key"
...
get_venv_bin_dir() {
	if [ "$PLATFORM" = "windows" ]; then
		echo "$HOME/.web-agent-env/Scripts"
	else
		echo "$HOME/.web-agent-env/bin"
	fi
}
...
install_web_agent() {
	log_info "Installing web-agent..."

	# Create or use existing virtual environment
	if [ ! -d "$HOME/.web-agent-env" ]; then
		# Use discovered Python command (e.g., python3.11) or fall back to version spec
		if [ -n "$PYTHON_CMD" ]; then
			uv venv "$HOME/.web-agent-env" --python "$PYTHON_CMD"
		else
			uv venv "$HOME/.web-agent-env" --python 3.11
		fi
	fi
	...
	web_agent_BRANCH="${web_agent_BRANCH:-main}"
	web_agent_REPO="${web_agent_REPO:-web-agent/web-agent}"
	log_info "Installing from GitHub: $web_agent_REPO@$web_agent_BRANCH"
	# Clone and install locally to ensure all dependencies are resolved
	local tmp_dir=$(mktemp -d)
	git clone --depth 1 --branch "$web_agent_BRANCH" "https://github.com/$web_agent_REPO.git" "$tmp_dir"
	uv pip install "$tmp_dir"
	rm -rf "$tmp_dir"

	log_success "web-agent installed"
}
...
write_install_config() {
	...
	# Write config file
	mkdir -p "$HOME/.web-agent"
	cat > "$HOME/.web-agent/install-config.json" << EOF
{
...
configure_path() {
	...
	# Check if already in PATH (web-agent-env matches both /bin and /Scripts)
	if grep -q "web-agent-env" "$shell_rc" 2>/dev/null; then
		log_info "PATH already configured in $shell_rc"
	else
		# Add to shell config (includes ~/.local/bin for cloudflared)
		echo "" >> "$shell_rc"
		echo "# web-agent" >> "$shell_rc"
...
configure_powershell_path() {
	...
	local scripts_path='\\.web-agent-env\\Scripts'
	...
	if echo "$current_path" | grep -q "web-agent-env"; then
	...
	if [ $? -eq 0 ]; then
		log_success "Added to Windows PATH: %USERPROFILE%\\.web-agent-env\\Scripts"
	else
		log_warn "Could not update PATH automatically. Add manually:"
		log_warn "  \$env:PATH += \";\$env:USERPROFILE\\.web-agent-env\\Scripts\""
	fi
}
...
run_setup() {
	...
	if [ -n "$API_KEY" ]; then
		web-agent setup --mode "$profile" --api-key "$API_KEY" --yes
	else
		web-agent setup --mode "$profile" --yes
	fi
}
...
validate() {
	...
	if web-agent doctor; then
		log_success "Installation validated successfully!"
		return 0
	else
		log_warn "Some checks failed. Run 'web-agent doctor' for details."
		return 1
	fi
}
...
print_next_steps() {
	...
	echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	echo ""
	log_success "web-agent installed successfully!"
	echo ""
	...
	if [ "$INSTALL_REMOTE" = true ] && [ -z "$API_KEY" ]; then
		...
		if [ "$PLATFORM" = "windows" ]; then
			echo "  \$env:web_agent_API_KEY=\"<your-api-key>\""
		else
			echo "  export web_agent_API_KEY=<your-api-key>"
		fi
		echo ""
		echo "  Get your API key at: https://web-agent.com"
		echo ""
	fi
	...
	if [ "$INSTALL_REMOTE" = true ] && [ -z "$API_KEY" ]; then
		...
		echo "  3. Try: web-agent open https://example.com"
	else
		echo "  2. Try: web-agent open https://example.com"
	fi

	echo ""
	echo "Documentation: https://docs.web-agent.com"
	echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	echo ""
}

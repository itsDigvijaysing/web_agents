"""Configuration system for web-agent with automatic migration support."""
...
class OldConfig:
	...
	@property
	def web_agent_CONFIG_FILE(self) -> Path:
		return self.web_agent_CONFIG_DIR / 'config.json'

	@property
	def web_agent_PROFILES_DIR(self) -> Path:
		path = self.web_agent_CONFIG_DIR / 'profiles'
		self._ensure_dirs()
		return path

	@property
	def web_agent_DEFAULT_USER_DATA_DIR(self) -> Path:
		return self.web_agent_PROFILES_DIR / 'default'

	@property
	def web_agent_EXTENSIONS_DIR(self) -> Path:
		path = self.web_agent_CONFIG_DIR / 'extensions'
		self._ensure_dirs()
		return path
...
class FlatEnvConfig(BaseSettings):
	...
	web_agent_DEBUG_LOG_FILE: str | None = Field(default=None)
	web_agent_INFO_LOG_FILE: str | None = Field(default=None)
	ANONYMIZED_TELEMETRY: bool = Field(default=True)
	web_agent_CLOUD_SYNC: bool | None = Field(default=None)
	web_agent_CLOUD_API_URL: str = Field(default='https://api.web-agent.com')
	web_agent_CLOUD_UI_URL: str = Field(default='')

	# Path configuration
	XDG_CACHE_HOME: str = Field(default='~/.cache')
	XDG_CONFIG_HOME: str = Field(default='~/.config')
	web_agent_CONFIG_DIR: str | None = Field(default=None)
	...
	web_agent_VERSION_CHECK: bool = Field(default=True)

	# MCP-specific env vars
	web_agent_CONFIG_PATH: str | None = Field(default=None)
	web_agent_HEADLESS: bool | None = Field(default=None)
	web_agent_ALLOWED_DOMAINS: str | None = Field(default=None)
	web_agent_LLM_MODEL: str | None = Field(default=None)

	# Proxy env vars
	web_agent_PROXY_URL: str | None = Field(default=None)
	web_agent_NO_PROXY: str | None = Field(default=None)
	web_agent_PROXY_USERNAME: str | None = Field(default=None)
	web_agent_PROXY_PASSWORD: str | None = Field(default=None)
...
class Config:
	...
	def _get_config_path(self) -> Path:
		...
		env_config = FlatEnvConfig()
		if env_config.web_agent_CONFIG_PATH:
			return Path(env_config.web_agent_CONFIG_PATH).expanduser()
		elif env_config.web_agent_CONFIG_DIR:
			return Path(env_config.web_agent_CONFIG_DIR).expanduser() / 'config.json'
		else:
			xdg_config = Path(env_config.XDG_CONFIG_HOME).expanduser()
			return xdg_config / 'webagent' / 'config.json'
	...
	def _load_config(self) -> dict[str, Any]:
		...
		if env_config.web_agent_HEADLESS is not None:
			config['browser_profile']['headless'] = env_config.web_agent_HEADLESS

		if env_config.web_agent_ALLOWED_DOMAINS:
			domains = [d.strip() for d in env_config.web_agent_ALLOWED_DOMAINS.split(',') if d.strip()]
			config['browser_profile']['allowed_domains'] = domains

		# Proxy settings (Chromium) -> consolidated `proxy` dict
		proxy_dict: dict[str, Any] = {}
		if env_config.web_agent_PROXY_URL:
			proxy_dict['server'] = env_config.web_agent_PROXY_URL
		if env_config.web_agent_NO_PROXY:
			# store bypass as comma-separated string to match Chrome flag
			proxy_dict['bypass'] = ','.join([d.strip() for d in env_config.web_agent_NO_PROXY.split(',') if d.strip()])
		if env_config.web_agent_PROXY_USERNAME:
			proxy_dict['username'] = env_config.web_agent_PROXY_USERNAME
		if env_config.web_agent_PROXY_PASSWORD:
			proxy_dict['password'] = env_config.web_agent_PROXY_PASSWORD
		if proxy_dict:
		...
		if env_config.web_agent_LLM_MODEL:
			config['llm']['model'] = env_config.web_agent_LLM_MODEL

		# Extension settings
		if env_config.web_agent_DISABLE_EXTENSIONS is not None:
			config['browser_profile']['enable_default_extensions'] = not env_config.web_agent_DISABLE_EXTENSIONS
...
def load_web_agent_config() -> dict[str, Any]:
	"""Load web-agent configuration for MCP components."""

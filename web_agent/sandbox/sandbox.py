...
from web_agent.sandbox.views import (
	BrowserCreatedData,
	ErrorData,
	LogData,
	ResultData,
...
def sandbox(
	...
	**env_vars: str,
	...
	Args:
	    web_agent_API_KEY: API key (defaults to web_agent_API_KEY env var)
	    ...
	    server_url: Sandbox server URL (defaults to https://sandbox.api.web-agent.com/sandbox-stream)
	...
	def decorator(
		...
		async def wrapper(*args, **kwargs) -> T:
			# 1. Get API key
			api_key = web_agent_API_KEY or os.getenv('web_agent_API_KEY')
			if not api_key:
				raise SandboxError('web_agent_API_KEY is required')
			...
			if needed_imports:
				needed_imports = 'from web_agent import Browser\n' + needed_imports
			else:
				needed_imports = 'from web_agent import Browser'
...
"""
			...
			url = server_url or 'https://sandbox.api.web-agent.com/sandbox-stream'

...
@cache
def get_web_agent_version() -> str:
	"""Get the web-agent package version using the same logic as Agent._set_web_agent_version_and_source"""
	try:
		...
		version = str(get_version('web-agent'))
		os.environ['LIBRARY_VERSION'] = version
		return version

	except Exception as e:
		logger.debug(f'Error detecting web-agent version: {type(e).__name__}: {e}')
		return 'unknown'


async def check_latest_web_agent_version() -> str | None:
	"""Check the latest version of web-agent from PyPI asynchronously.
	...
	try:
		async with httpx.AsyncClient(timeout=3.0) as client:
			response = await client.get('https://pypi.org/pypi/web-agent/json')

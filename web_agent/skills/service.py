"""Skills service for fetching and executing skills from the web agent API"""
...
from web_agent.skills.views import (
...
class SkillService:
	"""Service for managing and executing skills from the web agent API"""
	...
	def __init__(self, skill_ids: list[str | Literal['*']], api_key: str | None = None):
		...
		Args:
			...
			api_key: web agent API key (optional, will use env var if not provided)
		"""
		self.skill_ids = skill_ids
		self.api_key = api_key or os.getenv('web_agent_API_KEY') or ''
	...
	async def async_init(self) -> None:
		...
		try:
			...
			logger.info('Fetching skills from web agent API...')

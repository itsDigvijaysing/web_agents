...
web-agent is a standalone AI agent library, built on the foundation of the open-source **browser-use** project (MIT), that autonomously interacts with the web. It takes a user-defined task, navigates web pages using Chromium via CDP, processes HTML, and repeatedly queries a language model to decide the next action—until the task is completed. See `CLAUDE.md` for lineage details, known issues, and the active refactor plan.
...
- **Do not recommend `Chatwebagent` or any `use_cloud`/`cloud_*` browser parameters.** These pointed at a hosted cloud product that has been removed from this fork (see `CLAUDE.md`). Default to whichever provider the user has an API key for; this project's primary provider is **Groq** (`ChatGroq`, e.g. `openai/gpt-oss-20b` or `llama-3.1-8b-instant`). `Agent(llm=None)` now auto-resolves from env keys (Groq → OpenAI → Anthropic → Google) or raises a clear error.
</guidelines>

<web_agent_docs>

> The reference material below is inherited from browser-use's public docs. Sections describing removed cloud/sandbox features have been trimmed; the rest (Agent/Browser/Tools parameters) still reflects this codebase.
...
Create a `.env` file and add your API key. This project defaults to **Groq** if `GROQ_API_KEY` is set (see `AGENTS.md` guidelines above); OpenAI, Anthropic, and Google are also supported.
...
14+ providers supported under `web_agent/llm/` (OpenAI, Anthropic, Google, Groq, Azure, AWS Bedrock, Mistral, Cerebras, DeepSeek, Ollama, OpenRouter, Vercel, OCI). This project defaults to Groq — see the guidelines above.
...
Connect to any remote/hosted browser (self-hosted, another cloud provider, etc.) by passing its CDP URL directly — no built-in cloud provisioning exists in this fork:

```python  theme={null}
from web_agent import Agent, Browser, ChatOpenAI

browser = Browser(
    cdp_url="http://remote-server:9222"  # CDP URL from any provider
)
...
# Get Help
This is a standalone fork — see the [web_agents GitHub repo](https://github.com/itsDigvijaysing/web_agents) for issues. For general browser-use library concepts, browser-use's own [GitHub Issues](https://github.com/browser-use/browser-use/issues) may help.
...
```bash  theme={null}
git clone git@github.com:itsDigvijaysing/web_agents.git
cd web_agents

---
title: "Observability"
description: "Trace web agent's agent execution steps and capture browser session recording"
icon: "eye"
mode: "wide"
---
...
web agent has a native integration with [Laminar](https://laminar.sh) - open-source platform for monitoring and analyzing error patterns in AI agents.
Laminar SDK automatically captures **agent execution steps, costs and browser session recordings** of web agent agent.
Browser session recordings allows developers to see full video replay of the browser session, which is useful for debugging web agent agent.
...
Then, you simply initialize the Laminar at the top of your project and both web agent agent traces and session recordings will be automatically captured.
...
# web agent and any browser you use (local or remote)
...
To learn more about how you can trace and evaluate your web agent agent with Laminar, check out [Laminar docs](https://docs.lmnr.ai).

## web agent Cloud Authentication

web agent can sync your agent runs to the cloud for easy viewing and sharing. Authentication is required to protect your data.
...
python -m web_agent.cli auth
```

**Note**: Cloud sync is enabled by default. If you've disabled it, you can re-enable with `export web_agent_CLOUD_SYNC=true`.
...
rm ~/.config/webagent/cloud_auth.json
web-agent auth
```

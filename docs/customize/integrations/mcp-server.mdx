---
title: MCP Server
description: Connect AI models to web agent through the Model Context Protocol
---

web agent provides a hosted **Model Context Protocol (MCP)** server that enables AI assistants to control browser automation. Works with any HTTP-based MCP client, including Claude Code.

**MCP Server URL:** `https://api.web-agent.com/mcp`

This is an **HTTP-based MCP server** designed for cloud integrations and remote access. If you need a local stdio-based MCP server for Claude Desktop, use the free open-source version: `uvx web-agent --mcp`

## Quick Setup

### 1. Get API Key
Get your API key from the [web agent Dashboard](https://cloud.web-agent.com)
...
```bash
claude mcp add --transport http web-agent https://api.web-agent.com/mcp
```
</Tab>
...
```json
{
  "mcpServers": {
    "web-agent": {
      ...
      "args": [
        "mcp-remote",
        "https://api.web-agent.com/mcp",
        "--header",
        "X-web-agent-API-Key: your-api-key"
      ]
    }
  }
}
...
```json
{
  "mcpServers": {
    "web-agent": {
      "serverUrl": "https://api.web-agent.com/mcp",
      "headers": {
        "X-web-agent-API-Key": "your-api-key"
      }
    }
...
```bash
curl -X POST https://api.web-agent.com/oauth/register \
...
- **MCP Server URL**: `https://api.web-agent.com/mcp/chatgpt`
- **Client ID**: Paste the `client_id` from step 1

**Step 3: Authorize**

ChatGPT will redirect you to web agent's authorization page. Sign in and grant permission.
...
**Need help?**
Check our [Cloud Documentation](https://docs.cloud.web-agent.com) for detailed specifications.

---

## Local Self-Hosted Alternative

For users who want a free, self-hosted option, web-agent can run as a local MCP server on your machine. This requires your own OpenAI or Anthropic API keys but provides direct, low-level control over browser automation.
...
```bash
uvx --from 'web-agent[cli]' web-agent --mcp
```
...
```json
{
  "mcpServers": {
    "web-agent": {
      ...
      "args": ["--from", "web-agent[cli]", "web-agent", "--mcp"],
      "env": {
...
**CLI Extras Required:** The `--from web-agent[cli]` flag installs the CLI extras needed for MCP server support.
</Note>

#### Environment Variables

You can configure web-agent through environment variables:

- `OPENAI_API_KEY` - Your OpenAI API key (required)
- `ANTHROPIC_API_KEY` - Your Anthropic API key (alternative to OpenAI)
- `web_agent_HEADLESS` - Set to `false` to show browser window
- `web_agent_DISABLE_SECURITY` - Set to `true` to disable browser security features
...
- **`retry_with_web_agent_agent`** - Run a complete browser automation task with an AI agent (use as last resort when direct control fails)
...
"Go to GitHub, find the web-agent repository, and tell me about the latest release"
```

Claude will use the MCP server to execute these tasks through web-agent.
...
async def use_browser_mcp():
    # Connect to web-agent MCP server
...
Make sure you're using `--from 'web-agent[cli]'` in your uvx command:
```bash
uvx --from 'web-agent[cli]' web-agent --mcp
```
...
- Try setting `web_agent_HEADLESS=false` to see browser window
...
```bash
export web_agent_LOGGING_LEVEL=DEBUG
uvx --from 'web-agent[cli]' web-agent --mcp
```
...
- Explore the [examples directory](https://github.com/web-agent/web-agent/tree/main/examples/mcp) for more usage patterns
- Check out [MCP documentation](https://modelcontextprotocol.io/) to learn more about the protocol
- Join our [Discord](https://link.web-agent.com/discord) for support and discussions

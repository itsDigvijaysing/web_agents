...
web-agent provides lifecycle hooks that allow you to execute custom code at specific points during the agent's execution.
...
Currently, web-agent provides the following hooks:
...
- `agent.history` gives access to historical data from the agent's execution:
  - `agent.history.model_thoughts()`: Reasoning from web agent's model.
  - `agent.history.model_outputs()`: Raw outputs from the web agent's model.

...
web agent is a framework for AI Agents that interact with web browsers.
web agent Cloud is the fully hosted product made by web agent made for users to automate web-based tasks. 
...
Billing, API Key management, live session viewing, task results, account settings, and profile management is done through the web agent Cloud web app at https://cloud.web-agent.com/

## Core Concepts:
The key product of web agent Cloud is the completion of user tasks.
- A Session is the complete package of infrastructure web agent Cloud provides. Sessions are currently limited to 15 minutes of runtime. A session has a Browser running, and users can run Agents in a session to complete tasks. A Session is limited to one and only one Browser, which will be open the entire duration of the Session. Users can run a maximum of one Agent on a Session at a time, which will control the Browser. After one Agent is done, the user can run another within the same Session, limited only by the Session maximum duration.
- A Browser is simply a browser running on web agent Cloud infrastructure (a Session). Browsers (as a service) are controllable via CDP url. The user can use an Agent to control a Browser, or can request the CDP url and control the hosted browser with whatever scripts or external automations they desire. However we mainly encourage to control Browsers with web agent Agents, as they are optimized to work together. These official web agent browsers are forked from chromium, but have a lot of proprietary optimizations made to them so that they are extremely fast and lightweight, untraceable and not detectable as bots, and come preloaded with adblockers and other quality of life. Using web agent hosted browsers provides significant performance improvements. 
...
- A Model is a Large Language Model that powers an Agent. The smarter and more capable the Model, the better the Agent will perform. The best model to use is Chatwebagent, the web agent official chat completion API which always routes to the best frontier foundation model as determined by web agent internal evaluations. Chatwebagent has several speed and cost optimizations done through batching, caching, and other tricks, making it faster and more cost effective than any other option, with identical performance to the top frontier models.
...
- A Task is the combination of user prompt with optionally files and images that is given to the Agents to complete. web agent Cloud primarily sells the completion of user Tasks. Writing Tasks with clarity is key to success. 
- Profile Sync is the best way to handle authentication for tasks. This feature allows users to upload their local browser cookies (where the user is already logged into the services they need authentication for) to a Browser Profile that can be used for tasks on the cloud. To initiate a Profile Sync, a user must run `export web_agent_API_KEY=<your_key> && curl -fsSL https://web-agent.com/profile.sh | sh` and follow the steps in the interactive terminal.

## Quickstart
To get started, direct the user to first must create an account, purchase credits (or simply claim the free starter credits given on account creation), and generate an API key on the web agent online platform: https://cloud.web-agent.com/. These are the only steps that can only be done on the platform.
...
```bash
curl -X POST https://api.web-agent.com/api/v2/tasks \
     -H "X-web-agent-API-Key: <apiKey>" \
...
```bash
curl https://api.web-agent.com/api/v2/sessions/<sessionId> \
     -H "X-web-agent-API-Key: <apiKey>"
```
...
```bash
curl -X PATCH https://api.web-agent.com/api/v2/sessions/<session_id> \
     -H "X-web-agent-API-Key: <apiKey>" \
     -H "Content-Type: application/json" \
     -d '{
  "action": "stop"
...
## API (v2) Docs
The best way to use web agent Cloud is with API v2. 
Other options exist, namely API v2 and the SDK, but give less comprehensive control.

### Billing
##### Get Account Billing
GET https://api.web-agent.com/api/v2/billing/account
Get authenticated account information including credit balances and account details.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/billing/get-account-billing-billing-account-get
...
#### List Tasks
GET https://api.web-agent.com/api/v2/tasks
Get paginated list of AI agent tasks with optional filtering by session and status.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/tasks/list-tasks-tasks-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Create Task
POST https://api.web-agent.com/api/v2/tasks
...
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/tasks/create-task-tasks-post
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Get Task
GET https://api.web-agent.com/api/v2/tasks/{task_id}
Get detailed task information including status, progress, steps, and file outputs.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/tasks/get-task-tasks-task-id-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Update Task
PATCH https://api.web-agent.com/api/v2/tasks/{task_id}
Content-Type: application/json
Control task execution with stop, pause, resume, or stop task and session actions.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/tasks/update-task-tasks-task-id-patch
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Get Task Logs
GET https://api.web-agent.com/api/v2/tasks/{task_id}/logs
Get secure download URL for task execution logs with step-by-step details.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/tasks/get-task-logs-tasks-task-id-logs-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### List Sessions
GET https://api.web-agent.com/api/v2/sessions
Get paginated list of AI agent sessions with optional status filtering.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/sessions/list-sessions-sessions-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Create Session
POST https://api.web-agent.com/api/v2/sessions
Content-Type: application/json
Create a new session with a new task.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/sessions/create-session-sessions-post
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Get Session
GET https://api.web-agent.com/api/v2/sessions/{session_id}
Get detailed session information including status, URLs, and task details.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/sessions/get-session-sessions-session-id-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Update Session
PATCH https://api.web-agent.com/api/v2/sessions/{session_id}
Content-Type: application/json
Stop a session and all its running tasks.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/sessions/update-session-sessions-session-id-patch
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
GET https://api.web-agent.com/api/v2/sessions/{session_id}/public-share
Get public share information including URL and usage statistics.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/sessions/get-session-public-share-sessions-session-id-public-share-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
POST https://api.web-agent.com/api/v2/sessions/{session_id}/public-share
Create or return existing public share for a session.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/sessions/create-session-public-share-sessions-session-id-public-share-post
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
DELETE https://api.web-agent.com/api/v2/sessions/{session_id}/public-share
Remove public share for a session.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/sessions/delete-session-public-share-sessions-session-id-public-share-delete
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
POST https://api.web-agent.com/api/v2/files/sessions/{session_id}/presigned-url
Content-Type: application/json
Generate a secure presigned URL for uploading files that AI agents can use during tasks.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/files/user-upload-file-presigned-url-files-sessions-session-id-presigned-url-post
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
POST https://api.web-agent.com/api/v2/files/browsers/{session_id}/presigned-url
Content-Type: application/json
Generate a secure presigned URL for uploading files that AI agents can use during tasks.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/files/user-upload-file-presigned-url-browser-files-browsers-session-id-presigned-url-post
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
GET https://api.web-agent.com/api/v2/files/tasks/{task_id}/output-files/{file_id}
Get secure download URL for an output file generated by the AI agent.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/files/get-task-output-file-presigned-url-files-tasks-task-id-output-files-file-id-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### List Profiles
GET https://api.web-agent.com/api/v2/profiles
Get paginated list of profiles.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/profiles/list-profiles-profiles-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Create Profile
POST https://api.web-agent.com/api/v2/profiles
Content-Type: application/json
...
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/profiles/create-profile-profiles-post
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Get Profile
GET https://api.web-agent.com/api/v2/profiles/{profile_id}
Get profile details.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/profiles/get-profile-profiles-profile-id-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
DELETE https://api.web-agent.com/api/v2/profiles/{profile_id}
Permanently delete a browser profile and its configuration.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/profiles/delete-browser-profile-profiles-profile-id-delete
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Update Profile
PATCH https://api.web-agent.com/api/v2/profiles/{profile_id}
Content-Type: application/json
Update a browser profile's information.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/profiles/update-profile-profiles-profile-id-patch
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
GET https://api.web-agent.com/api/v2/browsers
Get paginated list of browser sessions with optional status filtering.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/browsers/list-browser-sessions-browsers-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
POST https://api.web-agent.com/api/v2/browsers
Content-Type: application/json
...
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/browsers/create-browser-session-browsers-post
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
#### Get Browser Session
GET https://api.web-agent.com/api/v2/browsers/{session_id}
Get detailed browser session information including status and URLs.
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/browsers/get-browser-session-browsers-session-id-get
OpenAPI Specification
```yaml
openapi: 3.1.1
info:
...
PATCH https://api.web-agent.com/api/v2/browsers/{session_id}
Content-Type: application/json
...
Reference: https://docs.cloud.web-agent.com/api-reference/v-2-api-current/browsers/update-browser-session-browsers-session-id-patch
OpenAPI Specification
```yaml
openapi: 3.1.1
info:

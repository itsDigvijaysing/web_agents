...
from web_agent.llm.messages import (
	SystemMessage as webagentSystemMessage,
)


class LangChainMessageSerializer:
	"""Serializer for converting between web-agent message types and LangChain message types."""
	...
	def _serialize_tool_call(tool_call: ToolCall) -> LangChainToolCall:
		"""Convert web-agent ToolCall to LangChain ToolCall."""
	...
	@overload
	@staticmethod
	def serialize(message: webagentSystemMessage) -> SystemMessage: ...
	...
	def serialize(message: BaseMessage) -> LangChainBaseMessage:
		"""Serialize a web-agent message to a LangChain message."""
		...
		elif isinstance(message, webagentSystemMessage):
	...
	def serialize_messages(messages: list[BaseMessage]) -> list[LangChainBaseMessage]:
		"""Serialize a list of web-agent messages to LangChain messages."""

...
from web_agent.llm.exceptions import ModelProviderError
from web_agent.llm.messages import BaseMessage
from web_agent.llm.views import ChatInvokeCompletion, ChatInvokeUsage
...
class ChatLangchain(BaseChatModel):
	"""
	A wrapper around LangChain BaseChatModel that implements the web-agent BaseChatModel protocol.

	This class allows you to use any LangChain-compatible model with web-agent.
	"""
	...
	async def ainvoke(
		...
		Args:
			messages: List of web-agent chat messages
		...
		# Convert web-agent messages to LangChain messages
		...
		try:
			...
			# Convert any LangChain errors to web-agent ModelProviderError
			raise ModelProviderError(

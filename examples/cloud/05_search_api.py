...
This example demonstrates the web agent Search API (BETA):
...
# Configuration
API_KEY = os.getenv('web_agent_API_KEY')
if not API_KEY:
	raise ValueError(
		'Please set web_agent_API_KEY environment variable. You can also create an API key at https://cloud.web-agent.com/new-api-key'
	)

BASE_URL = os.getenv('web_agent_BASE_URL', 'https://api.web-agent.com/api/v1')
TIMEOUT = int(os.getenv('web_agent_TIMEOUT', '30'))
...
async def demo_deep_website_analysis():
	...
	demo_start = time.time()
	url = 'https://docs.web-agent.com'
	query = 'web agent features, pricing, and API capabilities'
...
async def demo_real_time_vs_cached():
	...
	print('🔄 web agent Search API benefits:')
...
async def main():
	...
	print('🔍 web agent Cloud - Search API (BETA)')
	print('=' * 45)

"""
Key features:
1. web-agent and Playwright sharing the same Chrome instance via CDP
2. Take actions with Playwright and continue with web-agent actions
...
from web_agent import Agent, BrowserSession, ChatOpenAI, Tools
...
async def connect_playwright_to_cdp(cdp_url: str):
	"""
	Connect Playwright to the same Chrome instance web-agent is using.
...
async def playwright_fill_form(params: PlaywrightFillFormAction, browser_session: BrowserSession):
	...
	This demonstrates how to create web-agent actions that leverage Playwright's capabilities.
	"""
	try:
...
async def main():
	"""
	Main function demonstrating web-agent + Playwright integration with custom actions.
	"""
	print('🚀 Advanced Playwright + web-agent Integration with Custom Actions')
	...
	try:
		...
		# Step 3: Create web-agent session connected to same Chrome
		browser_session = BrowserSession(cdp_url=cdp_url)

		# Step 4: Create AI agent with our custom Playwright-powered tools
		agent = Agent(
			task="""
			Please help me demonstrate the integration between web-agent and Playwright:
			...
			This demonstrates how web-agent AI can orchestrate tasks while using Playwright's precise capabilities for specific operations.
			""",
		...
		# Step 5: Run the agent - it will use both web-agent actions and our custom Playwright actions

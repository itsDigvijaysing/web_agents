...
Example 1: Using OpenAI (default), with default task: 'go to reddit and search for posts about web-agent'
python command_line.py

Example 2: Using OpenAI with a Custom Query
python command_line.py --query "go to google and search for web-agent"
...
# Ensure local repository (web_agent) is accessible
...
def get_llm(provider: str):
	if provider == 'anthropic':
		from web_agent.llm import ChatAnthropic
		...
		from web_agent import ChatOpenAI
...
def parse_arguments():
	...
	parser.add_argument(
		'--query', type=str, help='The query to process', default='go to reddit and search for posts about web-agent'
	)
	parser.add_argument(
		'--provider',
		type=str,

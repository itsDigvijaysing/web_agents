#!/bin/bash
# Build script for web-agent base images
set -euo pipefail

# Configuration
REGISTRY="${DOCKER_REGISTRY:-webagent}"
...
if [[ "$PLATFORMS" == *","* ]] || [ "$PUSH" = "true" ]; then
    docker buildx inspect webagent-builder >/dev/null 2>&1 || \
        docker buildx create --name webagent-builder --use
    docker buildx use webagent-builder
fi

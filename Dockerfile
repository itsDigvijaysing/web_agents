...
# This is the Dockerfile for web-agent, it bundles the following dependencies:
#     python3, pip, playwright, chromium, web-agent and its dependencies.
# Usage:
#     git clone https://github.com/web-agent/web-agent.git && cd web-agent
#     docker build . -t webagent --no-cache
#     docker run -v "$PWD/data":/data webagent
#     docker run -v "$PWD/data":/data webagent --version
# Multi-arch build:
#     docker buildx create --use
#     docker buildx build . --platform=linux/amd64,linux/arm64--push -t webagent/webagent:some-tag
#
# Read more: https://docs.web-agent.com
...
LABEL name="webagent" \
    maintainer="Nick Sweeting <dockerfile@web-agent.com>" \
    description="Make websites accessible for AI agents. Automate tasks online with ease." \
    homepage="https://github.com/web-agent/web-agent" \
    documentation="https://docs.web-agent.com" \
    org.opencontainers.image.title="webagent" \
    org.opencontainers.image.vendor="webagent" \
    org.opencontainers.image.description="Make websites accessible for AI agents. Automate tasks online with ease." \
    org.opencontainers.image.source="https://github.com/web-agent/web-agent" \
    ...
    com.docker.extension.publisher-url="https://web-agent.com" \
    ...
    com.docker.extension.detailed-description='See here for detailed documentation: https://docs.web-agent.com' \
    com.docker.extension.changelog='See here for release notes: https://github.com/web-agent/web-agent/releases' \
...
RUN (echo "[i] Docker build for web agent $(cat /VERSION.txt) starting..." \
...
# Create non-privileged user for webagent and chrome
RUN echo "[*] Setting up $webagent_USER user uid=${DEFAULT_PUID}..." \
    && groupadd --system $webagent_USER \
    ...
    && mkdir -p /data \
    && mkdir -p /home/$webagent_USER/.config \
    && chown -R $webagent_USER:$webagent_USER /home/$webagent_USER \
    && ln -s $DATA_DIR /home/$webagent_USER/.config/webagent \
    && echo -e "\nwebagent_USER=$webagent_USER PUID=$(id -u $webagent_USER) PGID=$(id -g $webagent_USER)\n\n" \
    | tee -a /VERSION.txt
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked,id=apt-$TARGETARCH$TARGETVARIANT \
    ...
    && chown -R "$webagent_USER:$webagent_USER" "/home/${webagent_USER}/.config" \
    && ( \
...
RUN --mount=type=cache,target=/root/.cache,sharing=locked,id=cache-$TARGETARCH$TARGETVARIANT \
     echo "[+] Installing web-agent pip sub-dependencies..." \
     && ( \
        uv sync --all-extras --no-dev --no-install-project \
        && echo -e '\n\n' \
     ) | tee -a /VERSION.txt

# Copy the rest of the web-agent codebase
COPY . /app

# Install the web-agent package and all of its optional dependencies
RUN --mount=type=cache,target=/root/.cache,sharing=locked,id=cache-$TARGETARCH$TARGETVARIANT \
     echo "[+] Installing web-agent pip library from source..." \
     && ( \
        uv sync --all-extras --locked --no-dev \
        && python -c "import web_agent; print('web-agent installed successfully')" \
        && echo -e '\n\n' \
     ) | tee -a /VERSION.txt

RUN mkdir -p "$DATA_DIR/profiles/default" \
    && chown -R $webagent_USER:$webagent_USER "$DATA_DIR" "$DATA_DIR"/* \
    && ( \
